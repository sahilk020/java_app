...
import com.pay10.batch.FileUpiSettleValidator;
...
public class UpiSettleRecoFileValidator extends FileUpiSettleValidator {

	private static final Logger logger = LoggerFactory.getLogger(UpiSettleRecoFileValidator.class);
	...
	private boolean readPreviousValuesFromDB(Fields fields) throws DatabaseException {
		...
		if (isTransactionValid(fields)) {
			if (Constants.RECO_TRUE.getValue().equalsIgnoreCase(fields.get(FieldType.IS_RECO_SETTLED.getName()))) {
				...
				return false;
			}if (Constants.RECO_TRUE.getValue().equalsIgnoreCase(fields.get(FieldType.IS_RECO_FORCE_CAPRURED.getName()))) {
				logger.info("There is already a transaction with RECO and FORCE_CAPRURED state in DB for this SALE for transaction with status Failed with Pg_Ref_Num: " + pg_ref_num);
				return false;
			...
			} else if (Constants.RECO_FALSE.getValue().equalsIgnoreCase(fields.get(FieldType.IS_REFUND_CAPTURED.getName()))) {
				logger.info("Received transaction with REFUND and CAPTURED state in DB for Pg_Ref_Num: " + pg_ref_num);
				if ((!isAmountValid(fields, fields.get(FieldType.RECO_TOTAL_AMOUNT.getName())))) {
					return false;
				}
