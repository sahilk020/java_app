 ...
 */
public class FileRefundLineMapper<T> implements LineMapper<Fields>, InitializingBean {
	
	private static final Logger logger = LoggerFactory.getLogger(FileRefundLineMapper.class);
	...
	public Fields mapLine(String line, int lineNumber) throws Exception {
		...
		for (String fieldName : fileParameter) {
			value.put(fieldName, fieldSet.readString(fieldName).replace("\'", "").replaceAll("[^\\p{ASCII}]", ""));
		}
