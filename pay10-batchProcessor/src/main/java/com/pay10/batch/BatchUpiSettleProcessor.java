 ...
 */
public abstract class BatchUpiSettleProcessor implements ItemProcessor<Fields, Fields> {

	private static Logger logger = Logger.getLogger(BatchUpiSettleProcessor.class.getName());
	...
	private FileUpiSettleValidator fileSettleValidator;

	public void setFileSettleValidator(FileUpiSettleValidator fileSettleValidator) {
	...
	public Fields process(Fields fields) throws DatabaseException {
		try {
			...
			if (fields.get(FieldType.RESPONSE_CODE.getName()).equals("CR")) {
			...
			} else if (fields.get(FieldType.RESPONSE_CODE.getName()).equals("DR")) {
			...
			String message = "Error while setting data in fields ! BatchUpiSettleProcessor Class , process method !!";
	...
	protected Map<String, String> createNewRecoObject(Fields fields) throws SystemException {
		...
		try {
			...
			value.put(FieldType.RECO_AMOUNT.getName(), amount.formatAmount(
					previous.get(FieldType.RECO_AMOUNT.getName()), previous.get(FieldType.CURRENCY_CODE.getName())));
			value.put(FieldType.RECO_TOTAL_AMOUNT.getName(),
			...
			value.put(FieldType.ARN.getName(), "");
			...
			value.put(FieldType.RECO_CARD_HOLDER_TYPE.getName(),
					previous.get(FieldType.RECO_CARD_HOLDER_TYPE.getName()));
			...
			if (name.contains(Constants.YES_BANK_UPI.getValue())) {
				value.put(FieldType.RECO_ACQUIRER_TYPE.getName(), AcquirerType.YESBANKCB.getCode());
			} else {
				value.put(FieldType.RECO_ACQUIRER_TYPE.getName(), previous.get(FieldType.RECO_ACQUIRER_TYPE.getName()));
			}
			...
			String message = "Error while setting data in fields ! BatchUpiSettleProcessor Class , createNewRecoObject method !!";
			logger.error(message, exception);
			throw new SystemException(ErrorType.INTERNAL_SYSTEM_ERROR, exception, message);
		}
		return value;
	}

	/* Set required fields for RECO Transaction */
	protected Map<String, String> createNewRefundRecoObject(Fields fields) throws SystemException {
		Fields previous = fields.getPrevious();
		Map<String, String> value = new HashMap<String, String>();
		try {
			value.put(FieldType.TXN_ID.getName(), TransactionManager.getNewTransactionId());
			...
			String message = "Error while setting data in fields ! BatchUpiSettleProcessor Class , createNewRefundRecoObject method !!";
			logger.error(message, exception);
			throw new SystemException(ErrorType.INTERNAL_SYSTEM_ERROR, exception, message);
		}
		return value;
	...
	/*
	 * Set required fields for RECO PENDING Transaction protected Map<String,
	 * String> createNewRecoPendingObject(Fields fields) { Fields previous =
	 * fields.getPrevious(); Map<String, String> value = new HashMap<String,
	 * String>(); value.put(FieldType.TXN_ID.getName(),
	 * TransactionManager.getNewTransactionId());
	 * value.put(FieldType.PG_REF_NUM.getName(),
	 * fields.get(FieldType.PG_REF_NUM.getName()));
	 * value.put(FieldType.ORIG_TXN_ID.getName(),
	 * previous.get(FieldType.ORIG_TXN_ID.getName()));
	 * 
	 * if(previous.get(FieldType.ORIG_TXNTYPE.getName()) != null) {
	 * value.put(FieldType.ORIG_TXNTYPE.getName(),
	 * previous.get(FieldType.ORIG_TXNTYPE.getName())); } else {
	 * value.put(FieldType.ORIG_TXNTYPE.getName(),
	 * previous.get(FieldType.RECO_TXNTYPE.getName())); }
	 * 
	 * value.put(FieldType.RECO_AMOUNT.getName(),
	 * amount.formatAmount(previous.get(FieldType.RECO_AMOUNT.getName()),
	 * previous.get(FieldType.CURRENCY_CODE.getName())));
	 * value.put(FieldType.RECO_TOTAL_AMOUNT.getName(),amount.formatAmount(previous.
	 * get(FieldType.RECO_TOTAL_AMOUNT.getName()),
	 * previous.get(FieldType.CURRENCY_CODE.getName())));
	 * value.put(FieldType.ORIG_TXN_ID.getName(),
	 * previous.get(FieldType.ORIG_TXN_ID.getName()));
	 * value.put(FieldType.RECO_PAY_ID.getName(),
	 * previous.get(FieldType.RECO_PAY_ID.getName()));
	 * value.put(FieldType.CURRENCY_CODE.getName(),
	 * previous.get(FieldType.CURRENCY_CODE.getName()));
	 * value.put(FieldType.RECO_ORDER_ID.getName(),
	 * previous.get(FieldType.RECO_ORDER_ID.getName()));
	 * value.put(FieldType.OID.getName(), previous.get(FieldType.OID.getName()));
	 * value.put(FieldType.RRN.getName(),previous.get(FieldType.RRN.getName()));
	 * value.put(FieldType.RECO_ACQ_ID.getName(),previous.get(FieldType.RECO_ACQ_ID.
	 * getName()));
	 * value.put(FieldType.RECO_PAYMENT_TYPE.getName(),previous.get(FieldType.
	 * RECO_PAYMENT_TYPE.getName()));
	 * value.put(FieldType.MOP_TYPE.getName(),previous.get(FieldType.MOP_TYPE.
	 * getName()));
	 * value.put(FieldType.RECO_ACQUIRER_TYPE.getName(),previous.get(FieldType.
	 * RECO_ACQUIRER_TYPE.getName()));
	 * value.put(FieldType.CUST_NAME.getName(),previous.get(FieldType.CUST_NAME.
	 * getName()));
	 * value.put(FieldType.CUST_EMAIL.getName(),previous.get(FieldType.CUST_EMAIL.
	 * getName()));
	 * value.put(FieldType.CARD_MASK.getName(),previous.get(FieldType.CARD_MASK.
	 * getName()));
	 * value.put(FieldType.SURCHARGE_FLAG.getName(),previous.get(FieldType.
	 * SURCHARGE_FLAG.getName()));
	 * value.put(FieldType.PAYMENTS_REGION.getName(),previous.get(FieldType.
	 * PAYMENTS_REGION.getName()));
	 * value.put(FieldType.RECO_CARD_HOLDER_TYPE.getName(),previous.get(FieldType.
	 * RECO_CARD_HOLDER_TYPE.getName()));
	 * value.put(FieldType.RECO_PG_TXN_MESSAGE.getName(),previous.get(FieldType.
	 * RECO_PG_TXN_MESSAGE.getName()));
	 * value.put(FieldType.PG_DATE_TIME.getName(),previous.get(FieldType.
	 * PG_DATE_TIME.getName()));
	 * 
	 * value.put(FieldType.RESPONSE_CODE.getName(),
	 * ErrorType.RECO_SUCCESS.getResponseCode());
	 * value.put(FieldType.RESPONSE_MESSAGE.getName(),
	 * ErrorType.RECO_SUCCESS.getResponseMessage()); return value; }
	 */

	/* Set required fields for RECO REPORTING Transaction */
	protected Map<String, String> createNewRecoReportingObject(Fields fields) throws SystemException {
		Map<String, String> value = new HashMap<String, String>();
		Fields previous = fields.getPrevious();
		try {
			value.put(FieldType.TXN_ID.getName(), TransactionManager.getNewTransactionId());
			...
			String message = "Error while setting data in Reporting fields ! BatchUpiSettleProcessor Class , createNewRecoReportingObject method !!";
			logger.error(message, exception);
			throw new SystemException(ErrorType.INTERNAL_SYSTEM_ERROR, exception, message);
		}
		return value;
	...
	/* Set required fields for RECO REPORTING Transaction */
	protected Map<String, String> createNewRefundRecoReportingObject(Fields fields) throws SystemException {
		Map<String, String> value = new HashMap<String, String>();
		Fields previous = fields.getPrevious();
		try {
			value.put(FieldType.TXN_ID.getName(), TransactionManager.getNewTransactionId());
			...
			String message = "Error while setting data in Reporting fields  ! BatchUpiSettleProcessor Class , createNewRefundRecoReportingObject method !!";
			logger.error(message, exception);
			throw new SystemException(ErrorType.INTERNAL_SYSTEM_ERROR, exception, message);
		}
		return value;
