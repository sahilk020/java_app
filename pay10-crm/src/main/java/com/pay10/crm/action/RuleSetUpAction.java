...
public class RuleSetUpAction extends AbstractSecureAction {
	...
	private static Logger logger = LoggerFactory.getLogger(RuleSetUpAction.class.getName());
	boolean Valsubmit;
	...
	public String execute() {
		...
		if (validation) {
			...
			try {
				...
				//Checking FRM while setting RULE ENGINE
				String responseTdrCheck = fraudPreventionMongoService.checkFrmFromTdr(tempPayId);
				logger.info("responseFromTdr"+responseTdrCheck);

				if (!responseTdrCheck.equalsIgnoreCase("success")){
					setResponse(responseTdrCheck);
					addActionMessage(responseTdrCheck);
					flag = false;
				}
				else {
					...
					for (RouterRule routerRule : listData) {
						...
						routerRule.setMopType(MopType.getInstanceIgnoreCaseForRuleEngine(routerRule.getMopType()).getCode());

						routerRule.setPaymentType(
								PaymentType.getInstanceUsingStringValue(routerRule.getPaymentType()).getCode());
	...
	private boolean getvalidation() {
		for (RouterRule chargingDetail : listData) {
			...
			for (String acquirerString1 : acquirermap1) {
				...
				String valmop1 = chargingDetail.getMopType();
				String valmop=MopType.getInstanceIgnoreCaseForRuleEngine(valmop1).toString();
				String valpayment = chargingDetail.getPaymentType();
				String valpayid = chargingDetail.getMerchant();
				String Acqui=AcquirerType.getAcquirerName(valaquirer);
				logger.info("<<<<<<<<<<<<<<<<<<<<<<<<<<<<"+Acqui);

				boolean respCount = dao.getruleenginval(Acqui, valmop, valpayment, valpayid);
				Valsubmit = respCount;
				if (respCount == false) {
					break;
				}
