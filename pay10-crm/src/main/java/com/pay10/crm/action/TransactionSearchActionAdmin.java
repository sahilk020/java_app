...
@Component
public class TransactionSearchActionAdmin extends AbstractSecureAction {
	...
	private static Logger logger = LoggerFactory.getLogger(TransactionSearchActionAdmin.class.getName());
...
public void setRrn(String rrn) {
	...
	public String execute() {
		System.err.println("Deep Status : " + getStatus());
		logger.info("ipAddress===="+getIpAddress()+"totalAmount===="+getTotalAmount() + " FINAL STATUS : " + status);
		...
		//setDateFrom(DateCreater.toDateTimeformatCreater(dateFrom));
		//setDateTo(DateCreater.formDateTimeformatCreater(dateTo));
		System.out.println("Input Data check in TransationAdmin" +"channelName : "+channelName +" ||  totalAmount :"+totalAmount+ " || mopType  : "+ mopType + " || paymentType "+ paymentType);
		logger.info("Input Data check in TransationAdmin" +"channelName : "+channelName +" ||  totalAmount :"+totalAmount+ " || mopType  : "+ mopType + " || paymentType "+ paymentType + "|newDespositor is : |"+newDespositor);
		try {
			...
			System.err.println("Status1 : " + status);
			BigInteger bigInt = BigInteger.valueOf(txnReports.newSearchPaymentCount(transactionId, orderId, customerEmail, customerPhone, paymentType, 
					status, currency, transactionType, mopType, acquirer, merchantPayId, dateFrom, dateTo, sessionUser, start, length,tenantId,ipAddress,totalAmount,rrn,channelName,minAmount,maxAmount,columnName,logicalCondition,searchText,newDespositor,columnName1,logicalCondition1,searchText1,columnName2,logicalCondition2,searchText2));//,ipAddress,totalAmount));
			...
			aaData = encoder.encodeTransactionSearchObjNew(txnReports.newSearchPayment(transactionId, orderId, customerEmail, customerPhone, paymentType, 
					status, currency, transactionType, mopType, acquirer, merchantPayId, dateFrom, dateTo, sessionUser, start, length,tenantId,ipAddress,totalAmount,rrn,channelName,minAmount,maxAmount,columnName,logicalCondition,searchText,newDespositor,columnName1,logicalCondition1,searchText1,columnName2,logicalCondition2,searchText2));
			...
			logger.info("DATA Check for Admin Search "+aaData);
			System.out.println("DATA Check for Admin Search "+aaData);
	...
	public String customSearch() {
		...
		try {
			...
			if (sessionUser.getUserType() == UserType.ADMIN || sessionUser.getUserType() == UserType.SUBADMIN ) {
				BigInteger bigInt = BigInteger.valueOf(txnReports.newSearchPaymentCountCustom(transactionId, orderId, customerEmail, customerPhone, paymentType, 
						status, currency, transactionType, mopType, acquirer, merchantPayId, dateFrom, dateTo, sessionUser, start, length,tenantId));
				setRecordsTotal(bigInt);
				if (getLength() == -1) {
					setLength(getRecordsTotal().intValue());
				}
				aaData = encoder.encodeTransactionSearchObjNew(txnReports.newSearchPaymentCustom(transactionId, orderId, customerEmail, customerPhone, paymentType, 
						status, currency, transactionType, mopType, acquirer, merchantPayId, dateFrom, dateTo, sessionUser, start, length,tenantId));
				recordsFiltered = recordsTotal;
				aaData = aaData.stream().sorted(new Comparator<TransactionSearchNew>() {
	...
	public void validate() {
		logger.info("Validate function final  : " + status);
		...
		if (validator.validateBlankField(getTenantId())  || getTenantId().equals(CrmFieldConstants.ALL.getValue())) {
		} else if (!validator.validateField(CrmFieldType.TENANT_ID, getTenantId())) {
			addFieldError(CrmFieldType.TENANT_ID.getName(), ErrorType.INVALID_FIELD.getResponseMessage());
		}
		...
		/*
		 * if(StringUtils.isNoneBlank(status)&&status.
		 * equalsIgnoreCase("Request Accepted")) {
		 * 
		 * } else { if (validator.validateBlankField(getStatus())) { } else if
		 * (!validator.validateField(CrmFieldType.ALIAS_STATUS, getStatus())) {
		 * addFieldError(CrmFieldType.ALIAS_STATUS.getName(),
		...
		if (!validator.validateBlankField(getDateTo())) {
			...
			} else if (DateCreater.diffDate(getDateFrom(), getDateTo()) > 90) {
				addFieldError(CrmFieldType.DATE_FROM.getName(), CrmFieldConstants.DATE_RANGE.getValue());
			}
		}
		logger.info("Validate function final END  : " + status);
	}
