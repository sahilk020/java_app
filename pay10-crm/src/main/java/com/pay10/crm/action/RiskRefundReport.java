...
public class RiskRefundReport extends AbstractSecureAction {
	...
	private static Logger logger = LoggerFactory.getLogger(RiskRefundReport.class.getName());
	...
	public String execute() {
		...
		try {
			...
			BigInteger bigInt = BigInteger.valueOf(txnReports.riskListReportCount(acquirer, payId, dateFrom, dateTo, start, length));//,ipAddress,totalAmount));
			setRecordsTotal(bigInt);
			if (getLength() == -1) {
				setLength(getRecordsTotal().intValue());
			}
			aaData = encoder.encodeTransactionSearchObjNew(txnReports.riskListReport(acquirer, payId, dateFrom, dateTo, start, length));
