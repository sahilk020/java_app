...
public class CustomerAddressActionAdmin extends AbstractSecureAction {

	private static final long serialVersionUID = 2845115077638261715L;
	private User sessionUser = new User();
	private static Logger logger = LoggerFactory.getLogger(CustomerAddressActionAdmin.class.getName());
	...
	public void setTrailData(List<TransactionSearch> trailData) {
		...
		/*
		 * this.trailData = trailData.stream().sorted(new
		 * Comparator<TransactionSearch>() {
		 * 
		 * @Override public int compare(TransactionSearch o1, TransactionSearch o2) { //
		 * Done By chetan nagaria for change in settlement process to mark transaction
		 * as RNS // if(!o2.getStatus().equalsIgnoreCase(StatusType.SETTLED.getName())&&
		 * o1.getTxnType().equalsIgnoreCase(TransactionType.REFUND.getName())) {
		 * if(!o2.getStatus().equalsIgnoreCase(StatusType.SETTLED_SETTLE.getName())&&
		 * o1.getTxnType().equalsIgnoreCase(TransactionType.REFUND.getName())) { return
		 * -1; } return 0; } }).collect(Collectors.toList());
		 */
	}
