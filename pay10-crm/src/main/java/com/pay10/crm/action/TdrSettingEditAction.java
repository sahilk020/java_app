 ...
 *
 */
public class TdrSettingEditAction extends AbstractSecureAction {
	...
	private static Logger logger = LoggerFactory.getLogger(TdrSettingEditAction.class.getName());
	...
	public String execute() {
		try {
			TdrSetting prevChargingDetailsFromDb = tdrSettingDao.find(tdr.getId());
			String prevValue = prevChargingDetailsFromDb != null ? mapper.writeValueAsString(prevChargingDetailsFromDb)
					: null;
			
			setResponseMessage(editChargingDetails.editTdrSetting(tdr,emailId ,userType, loginUserEmailId));
			logger.info("Response :"+getResponseMessage());
			TdrSetting latestActivechargingDetailFromDb =null;
			if(prevChargingDetailsFromDb!=null) {
				latestActivechargingDetailFromDb = tdrSettingDao.find(prevChargingDetailsFromDb.getId());
			}else {
				latestActivechargingDetailFromDb=tdr;
			}
			...
			AuditTrail auditTrail = new AuditTrail(mapper.writeValueAsString(latestActivechargingDetailFromDb),
					prevValue, actionMessagesByAction.get("editTdrSetting"));
