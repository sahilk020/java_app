...
public class ResellerTransactionSearchAction extends AbstractSecureAction{
	...
	private static Logger logger = LoggerFactory.getLogger(ResellerTransactionSearchAction.class.getName());
...
public void setRrn(String rrn) {
	...
	public String execute() {
		...
		try {
			...
			logger.info("Inside ResellerTransactionSearchAction , merchantPayId = " + merchantPayId);
			
			BigInteger bigInt = BigInteger.valueOf(txnReports.newResellerSearchPaymentCount(transactionId, orderId, customerEmail, customerPhone, paymentType, 
					status, currency, transactionType, mopType, acquirer, merchantPayId, dateFrom, dateTo, sessionUser, start, length,tenantId,ipAddress,totalAmount,rrn,channelName,minAmount,maxAmount,columnName,logicalCondition,searchText,newDespositor,columnName1,logicalCondition1,searchText1,columnName2,logicalCondition2,searchText2,smaId, maId, agentId));//,ipAddress,totalAmount));
			logger.info("Inside ResellerTransactionSearchAction , bigInt=========== = " + bigInt);

			setRecordsTotal(bigInt);
			if (getLength() == -1) {
				setLength(getRecordsTotal().intValue());
			}
			aaData = encoder.encodeTransactionSearchObjNew(txnReports.newResellerSearchPayment(transactionId, orderId, customerEmail, customerPhone, paymentType, 
					status, currency, transactionType, mopType, acquirer, merchantPayId, dateFrom, dateTo, sessionUser, start, length,tenantId,ipAddress,totalAmount,rrn,channelName,minAmount,maxAmount,columnName,logicalCondition,searchText,newDespositor,columnName1,logicalCondition1,searchText1,columnName2,logicalCondition2,searchText2,smaId, maId, agentId));
	...
	public void validate() {
		...
		if(StringUtils.isNoneBlank(status)&&status.equalsIgnoreCase("REFUND_INITIATED")) {
