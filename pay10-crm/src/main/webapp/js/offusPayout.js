...
var Offus = function(merchant, payment_type, mop, transaction_type, acquirer, onUsFlag , cardHolderType ,paymentsRegion ){
    this.merchant = merchant;
    this.channel = payment_type;
    this.currency = mop;
            ...
            for(var ou_paymentType_key = 0; ou_paymentType_key < ou_paymentType.length; ou_paymentType_key ++){
                ...
                for(var ou_mopType_key = 0; ou_mopType_key < ou_mopType.length; ou_mopType_key ++){
                    ...
                    for(var ou_txnType_key = 0; ou_txnType_key < ou_txnType.length ; ou_txnType_key ++ ){
                        ...
                        ou_row = new Offus(merchant,ou_paymentType[ou_paymentType_key], ou_mopType[ou_mopType_key], ou_txnType[ou_txnType_key], ou_Acquirer.join(', '), onUsFlag , cardHolderType ,paymentsRegion);

                        match = false;
                        for(var a = 0; a < CurrentOffusListLength; a++){
                        	if( ( ou_row.merchant === OffusList[a].merchant || ou_row.merchant != "ALL MERCHANTS" && OffusList[a].merchant == "ALL MERCHANTS" )  && (ou_row.paymentType === OffusList[a].paymentType) && (ou_row.mopType === OffusList[a].mopType) && (ou_row.transactionType === OffusList[a].transactionType) && (ou_row.onUsFlag === OffusList[a].onUsFlag) && (ou_row.cardHolderType === OffusList[a].cardHolderType) && (ou_row.paymentsRegion === OffusList[a].paymentsRegion) )
                        	{
                        		match = true;
                        		matchNumber++;
                        	}
                ...
                for(var ou_mopType_key = 0; ou_mopType_key < ou_mopType.length; ou_mopType_key ++){
                    ...
                    for(var ou_txnType_key = 0; ou_txnType_key < ou_txnType.length ; ou_txnType_key ++ ){
                    	...
                    	for(var ou_Acquirer_key = 0; ou_Acquirer_key < ou_Acquirer.length ; ou_Acquirer_key ++ ){
                    		...
                    		ou_row = new Offus(merchant, ou_paymentType[ou_paymentType_key], ou_mopType[ou_mopType_key], ou_txnType[ou_txnType_key], ou_Acquirer[ou_Acquirer_key], onUsFlag)
                            match = false;
                            for(var a = 0; a < CurrentOffusListLength; a++){
...
$("#acquirerFinal").on("select2:select", function (evt) {
	...
	OffusList = data.routerRules;
			...
			if(OffusList.length > 0){
				acqH="<input type='checkbox' class='selectAllcountry' id='myCheck'  name='deleteRule' onClick='toggle(this)' value=Select All'/><label>Select All</label"
					butt="<button type='button' id='but' class='btn btn-danger'  name='deleteRule' style='position:relative;left:900px;top:-8px;'onclick='myFun()' >Delete ALL</button>"
		...
		document.getElementById("del1").innerHTML=butt;
		         ...
		         for(var i = 0; i < OffusList.length; i++){
		        	 if(OffusList[i].onUsFlag == false){
				         $('.offus_table').show();
		        		 $('.offus_table').append('<tr class="boxtext"><td><input type="checkbox" class="chkCountry" onclick="mychecktest()" id="chbox" name="sport" value="'+OffusList[i].id+'"></td><td align="left" valign="middle">' + OffusList[i].merchant + '</td><td align="left" valign="middle">' + OffusList[i].channel + '</td><td align="left" valign="middle">' + OffusList[i].currency + '</td><td align="left" valign="middle">' + OffusList[i].transactionType + '</td><td align="left" valign="middle">' + OffusList[i].acquirerMap + '<div class="AcquirerListTemp_'+OffusList[i].id+'"></div></td><td  align="left" valign="middle">' + OffusList[i].paymentsRegion + '</td><td align="left" valign="middle"> ' + OffusList[i].cardHolderType + '</td><td align="left" valign="top">' + '<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#myModal" id="editModel" onClick="editMethod(this)" data-id="'+OffusList[i].id+'" >&times; Edit</button> &nbsp; <button type="button" class="btn btn-danger remove-row"  name="deleteRule" data-id="'+OffusList[i].id+'">&times; Remove</button></td></tr>')
		        	 }else{
						 document.getElementById("loading").style.display = "none";

				         $('.onus_table').show();
		        		 $('.onus_table').append('<tr class="boxtext"><td align="left" valign="middle">' + OffusList[i].merchant + '</td><td align="left" valign="middle">' + OffusList[i].acquirerMap + '</td><td align="left" valign="middle">' + OffusList[i].channel + '</td><td align="left" valign="middle">' + OffusList[i].currency + '</td><td align="left" valign="middle">' + OffusList[i].transactionType + '</td><td align="left" valign="top">' + '<button type="button" disabled="disabled" name="deleteRule" class="btn btn-danger remove-row" data-id="'+OffusList[i].id+'">&times; Remove</button></td></tr>')
		        	 }
		         }
	 ...
	 $('#offus_section input').click(function(){
		 debugger
	    	if($('#offus_section input:checked').length > 0){
	    		...
	    		if( ($('#offus_section input[name="txnType"]').is(':checked')) && ($('#offus_section input[name="paymentType"]').is(':checked')) && ($('#offus_section input[name="mopType"]').is(':checked')) && ($('.AcquirerList input[name^="Acquirer"]').is(':checked')) ){
		  ...
		  $("#offus_merchant").on('change', function(){
		  		...
		  		//	localStorage.clear();
		  			...
		  			for(var itm =0; itm< acquirerSize;itm++)
		  			{
		  			  ...
		  			  acqHtml1 = acqHtml1+"<div id='wwgrp_acquirer' class='wwgrp'><div id='wwctrl_acquirer' class='checkbox' width='100%'><input type='radio'   onclick='getpaymenttype1(this.value)' id='acquirer1"+data.acquirerList[itm]+"' name='Acquirer1' value='"+data.acquirerList[itm]+"' ><label for='Acquirer1"+data.acquirerList[itm]+"'>"+data.acquirerList[itm]+"</label></div></div>"
...
//    		if( ($('#onus_section input[name="txnType"]').is(':checked')) && ($('#onus_section input[name="paymentType"]').is(':checked')) && ($('#onus_section input[name="mopType"]').is(':checked')) && ($('#onus_section input[name="acquirer"]').is(':checked')) ){
   ...
   //Offus Edit Funtion Start Here
	...
	$('.product-spec').on('click', '.edit-row', function(events){
		...
		$('.AcquirerListTemp_'+index+' + .acquirerCloneBtn').on('click', function(){
			...
			acquirerClone = acquirerClone.replaceAll(newAcq, ogAcq);
			$('.AcquirerListTemp_'+index).append(acquirerClone);
...
function editMethod(cont){
	...
	var currency = $($(cont).closest('tr').find('td')[3]).html();
	var channel = $($(cont).closest('tr').find('td')[2]).html();
	console.log("Merchant :"+merchant+" Currency :"+currency+" Channel :"+channel);
	...
	$.ajax({
		type:"GET",
		url:domain+"/crmws/ruleEnginPayout/getAcquirerForSmartRouterPayout?merchantName="+merchant+"&currency="+currency+"&channel="+channel,
