...
<div class="toolbar" id="kt_toolbar">
				...
				<div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
					...
					<div data-kt-swapper="true" data-kt-swapper-mode="prepend"
						...
						<!--begin::Title-->
						<h1 class="d-flex align-items-center text-dark fw-bold my-1 fs-3">PDF Merger</h1>
						<!--end::Title-->
						<!--begin::Separator-->
						...
						<ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-1">
							...
							<!--end::Item-->
							<!--begin::Item-->
							<li class="breadcrumb-item text-dark">Merge PDF Files</li>
							<!--end::Item-->
						</ul>
						<!--end::Breadcrumb-->
					</div>
...
<s:form action="PDFMergerAction" id="form">
<s:hidden name="id" id = "id" />
    ...
    <input type="hidden" name="name"  id="name"  value ="<s:property value='%{#session.USER.emailId}'/>"></input>
      <span id= "excelError" class ="error"></span>
      <button type="button" class="btn btn-primary btn-xs" style="margin-top:20px;"
      name="submitFile" id="submitFile" onClick="saveFile()">Merge</button>
      <!--  <button type="button" class="btn btn-primary btn-xs" style="margin-top:20px;"
      name="submitFile" id="submitFile" onClick="saveFile()">View All</button> -->
...
<script>
 ...
 function saveFile(){
   ...
   if (!(/\.(pdf)$/i).test(file1)) {
       alert('Please upload valid pdf file .pdf');
       $(file1).val('');
   }
   if (!(/\.(pdf)$/i).test(file2)) {
       alert('Please upload valid pdf file .pdf');
   ...
   $.ajax({
      url: domain+"/crmws/pdf/mergePDF",
    // url:   "http://localhost:8080/crmws/pdf/mergePDF",
      ...
      data: data,
        processData: false,
          ...
          success: function (data) {
        	  alert("PDF merged sucessfully!");
				window.location.reload();
          },
          error: function(data, textStatus, jqXHR) {
        	  
  			if(data.responseText && JSON.parse(data.responseText).respmessage){
  				alert("Error while merging uploading PDF file");
  				window.location.reload();
  			}
