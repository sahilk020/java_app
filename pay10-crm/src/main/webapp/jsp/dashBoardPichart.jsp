...
var aquirer = /*new Date(currentDate.setDate(last))/*;

function drawChart() {  $.ajax({url:"",type:"",data:{},success:})
	var token = document.getElementsByName("token")[0].value;
	$.ajax({
		url : "http://localhost:8080/pgws/dashboardPiChart",
		type : "POST",
		data : {
			dateFrom : dateFrom,
			dateTo : dateTo,
...
function drawBarChart() {
	var token = document.getElementsByName("token")[0].value;
	$.ajax({
		url : "localhost:8080/pgws/dashboardPiChart",
		type : "POST",
		data : {
			dateFrom : dateFrom,
			dateTo : dateTo,
