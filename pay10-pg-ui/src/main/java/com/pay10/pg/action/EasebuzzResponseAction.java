...
public class EasebuzzResponseAction extends AbstractSecureAction implements ServletRequestAware {

	private static Logger logger = LoggerFactory.getLogger(EasebuzzResponseAction.class.getName());
		...
		try {
			...
			for (Entry<String, String[]> entry : fieldMapObj.entrySet()) {
				try {
					...
					logger.error("1- EaseBuzzResponseAction Exception", classCastException);
				}
			}
			...
			logger.info("Response received from Easebuzz: " + resJson.toString());
			...
			if (StringUtils.isBlank(txnKey)) {
				...
				logger.info("Again InternalRequestFields In Easebuzz Response Action : "+internalRequestFields);
				 ...
				 */
				logger.info("Return URL In Easebuzz Response Action : " + paramMap.get(FieldType.RETURN_URL.getName()));
				...
				logger.info(" cust ipaddress in EASEBUZZ Acquirer = {}", ipAddress);
			 ...
			 * ())); fields = fieldsDao.getPreviousForPgRefNum(pgRef);
			 * logger.info("fields "+fields.getFieldsAsString()); //fields =
			 * fieldsDao.getPreviousForPgRefNum("1101611105165653"); String
			 ...
			 * if (StringUtils.isBlank(txnKey)) {
			 * 
			 * if (StringUtils.isNotBlank(pgRef)) { String pgRefNum = pgRef;
			 * 
			 * fields = fieldsDao.getPreviousForPgRefNum(pgRefNum); String
			 * internalRequestFields =
			 ...
			 * paramMap.get(FieldType.ORDER_ID.getName())+" in EASEBUZZ");
			 * fields.put(FieldType.IS_MERCHANT_HOSTED.getName(),
			 * paramMap.get(FieldType.IS_MERCHANT_HOSTED.getName()));
			 * sessionMap.put(FieldType.IS_MERCHANT_HOSTED.getName(),
			 ...
			 * paramMap.get(FieldType.ORDER_ID.getName())+" in EASEBUZZ"); }
			 * 
			 * Map<String, String> paymentMap = new HashMap<String, String>(); paymentMap =
			 * getTxnKey(fields);
			 * 
			 * txnKey = paymentMap.get(FieldType.TXN_KEY.getName()); merchantId =
			 * paymentMap.get(FieldType.MERCHANT_ID.getName());
			 * fields.put(FieldType.MERCHANT_ID.getName(),
			 * paymentMap.get(FieldType.MERCHANT_ID.getName()));
			 * fields.put(FieldType.TXN_KEY.getName(),
			 * paymentMap.get(FieldType.TXN_KEY.getName())); }
			 * 
			 * }
			 */

			fields.put(FieldType.EASEBUZZ_RESPONSE_FIELD.getName(), resJson.toString());
			fields.put(FieldType.TXN_KEY.getName(), txnKey);
			fields.put(FieldType.MERCHANT_ID.getName(), resJson.get("key").toString());
			fields.logAllFields("Easebuzz Response Recieved :");
			...
			fields.put(FieldType.ACQUIRER_TYPE.getName(), AcquirerType.EASEBUZZ.getCode());
			...
			Map<String, String> response = transactionControllerServiceProvider.transact(fields,
					Constants.TXN_WS_EASEBUZZ_PROCESSOR.getValue());   
			...
			logger.error("2-  EaseBuzzResponseAction Exception", exception);
			return ERROR;
		}
		return Action.NONE;
	}
		...
		if (accounts == null || accounts.size() == 0) {
			...
			for (Account accountThis : accounts) {
				if (accountThis.getAcquirerName().equalsIgnoreCase(
						AcquirerType.getInstancefromCode(AcquirerType.EASEBUZZ.getCode()).getName())) {
					account = accountThis;
					break;
				}
			}
