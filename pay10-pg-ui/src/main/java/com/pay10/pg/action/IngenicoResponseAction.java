...
import com.pay10.pg.core.ingenico.util.TransactionResponseBean;
 ...
 *
 */
public class IngenicoResponseAction extends AbstractSecureAction implements ServletRequestAware {

	private static Logger logger = LoggerFactory.getLogger(IngenicoResponseAction.class.getName());
	...
	public String execute() {
		try {
			...
			String mid  = requestMap.get("tpsl_mrct_cd");
			
			
			logger.info("Response received from ingenico: " + transData);
			...
			transactionResponseBean.setResponsePayload(transData);
			transactionResponseBean.setKey(txnKey.getBytes());
			transactionResponseBean.setIv(iv.getBytes());
			
			decrytedString = transactionResponseBean.getResponsePayload();
			
			//bobUtil.decryptText(txnKey, transData);

			Fields fields = new Fields();
			fields.put(FieldType.INGENICO_RESPONSE_FIELD.getName(), decrytedString);
			fields.logAllFields("Ingenico Response Recieved :");
			...
			fields.logAllFields("Updated Ingenico 3DS Recieved Map TxnType = " + fields.get(FieldType.TXNTYPE.getName()) + " " + "Txn id = "+ fields.get(FieldType.TXN_ID.getName()));
			fields.put(FieldType.ACQUIRER_TYPE.getName(), AcquirerType.INGENICO.getCode());
			...
			Map<String, String> response = transactionControllerServiceProvider.transact(fields,
					Constants.TXN_WS_INGENICO_PROCESSOR.getValue());
