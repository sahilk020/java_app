...
@Service("matchMoveProcessor")
public class MatchMoveProcessor implements Processor {
	
	
	private static Logger logger = LoggerFactory.getLogger(MatchMoveProcessor.class.getName());
	
	@Autowired
	private MatchMoveIntegrator iMudraIntegrator;
	...
	public void process(Fields fields) throws SystemException {
		if ((fields.get(FieldType.TXNTYPE.getName()).equals(TransactionType.NEWORDER.getName())
			...
			// New Order Transactions are not processed by MatchMoveWallet
			return;
		}

		if (!fields.get(FieldType.ACQUIRER_TYPE.getName()).equals(AcquirerType.MATCHMOVE.getCode())) {
			return;
		...
		iMudraIntegrator.process(fields);
