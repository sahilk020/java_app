...
@Service
public class CityunionBankNBTransformer {
	
	private static Logger logger = LoggerFactory.getLogger(CityunionBankNBTransformer.class.getName());

	private Transaction transaction = null;
	public CityunionBankNBTransformer(Transaction transaction) {
	...
	public void updateResponse(Fields fields) {
		...
		logger.info("log for tarsnaction ={}",transaction);
				logger.info("Looking For Acquirer {} ErrorCode Mapping Status {}","CITYUNIONBANK",transaction.getPaid());
				ErrorMappingDTO errorMappingDTO = new ErrorMappingDAOImpl().getErrorMappingByAcqCode
						(transaction.getPaid(), "CITYUNIONBANK");
				...
				if(null != errorMappingDTO) {
					...
					logger.info("ErrorCodeMapping not fount for CITYUNIONBANK Acquirer");
