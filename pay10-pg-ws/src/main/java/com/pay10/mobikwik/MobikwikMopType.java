...
public enum MobikwikMopType {
	...
	private MobikwikMopType(String bankName, String code, String bankCode){
	...
	public static MobikwikMopType getInstance(String name){
		MobikwikMopType[] mopTypes = MobikwikMopType.values();
		for(MobikwikMopType mopType : mopTypes){
	...
	public static List<MobikwikMopType> getGetMopsFromSystemProp(String mopsList){

		List<String> mopStringList= (List<String>) Helper.parseFields(PropertiesManager.propertiesMap.get(mopsList));

		List<MobikwikMopType> mops = new ArrayList<MobikwikMopType>();

		for(String mopCode:mopStringList){
			MobikwikMopType mop = getmop(mopCode);
			mops.add(mop);
		}
		return mops;
	}
	...
	public static String getmopName(String mopCode){
		MobikwikMopType mopType = MobikwikMopType.getmop(mopCode);		
		if(mopType == null) {
			return "";
		}
	...
	public static String getBankCode(String code){
		MobikwikMopType mopType = MobikwikMopType.getmop(code);		
		if(mopType == null) {
			return "";
		}
	...
	public static MobikwikMopType getmop(String mopCode){
		MobikwikMopType mopObj = null;
		if(null!=mopCode){
			for(MobikwikMopType mop:MobikwikMopType.values()){
	...
	public static MobikwikMopType getInstanceIgnoreCase(String name){
		MobikwikMopType[] mopTypes = MobikwikMopType.values();
		for(MobikwikMopType mopType : mopTypes){
