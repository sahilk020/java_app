...
@Service
public class IPayEnquiryResponseProcessor {
	
	private static Logger logger = LoggerFactory.getLogger(IPayEnquiryResponseProcessor.class.getName());
	...
	public void process(Fields fields) throws SystemException {
		...
		fields.put(FieldType.TXNTYPE.getName(), fields.getPrevious().get(FieldType.TXNTYPE.getName()));
	}

	private void transformResponse(Fields fields) throws SystemException {
		String decryptedValue = decrypt(fields);
		logger.info("Status Enquiry response after decryption: " + decryptedValue);
		...
		if (!isCheckSumMatching(decryptedValue)) {
			...
			logger.info("Checksum for Status Enquiry Response is not matching");
		}
	}
	...
	private String decrypt(Fields fields) {
		...
		logger.info("Status Enquiry response before decryption: " + encrypted);
	...
	private void mapFields(String decryptedValue, Fields fields) {
		...
		if (values.length == 7) {
			...
			fields.put(FieldType.INTERNAL_ORIG_TXN_TYPE.getName(), TransactionType.ENQUIRY.getName());
		}
		else {
