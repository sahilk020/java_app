...
@Service("iciciMpgsProcessor")
public class IciciMpgsProcessor implements Processor {
	
	@Autowired
	private IciciMpgsIntegrator iciciMpgsIntegrator;
	...
	public void process(Fields fields) throws SystemException {
		...
		if ((fields.get(FieldType.TXNTYPE.getName()).equals(TransactionType.NEWORDER.getName())
			...
			// New Order Transactions are not processed by Icici MPGS
			return;
		}

		if (!fields.get(FieldType.ACQUIRER_TYPE.getName()).equals(AcquirerType.ICICI_MPGS.getCode())) {
			return;
		}
		...
		iciciMpgsIntegrator.process(fields);
