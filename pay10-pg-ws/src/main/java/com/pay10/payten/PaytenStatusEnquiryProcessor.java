...
@Service
public class PaytenStatusEnquiryProcessor {

	@Autowired
	@Qualifier("paytenTransactionConverter")
	private TransactionConverter converter;

	private static Logger logger = LoggerFactory.getLogger(PaytenStatusEnquiryProcessor.class.getName());
	...
	public void enquiryProcessor(Fields fields) {
		...
		try {
			...
			logger.error("Exception in decrypting status enquiry response for pay10 ", e);
		}
	...
	public String statusEnquiryRequest(Fields fields) {
		...
		try {
			...
			logger.error("Exception in preparing pay10 Status Enquiry Request", e);
			return null;
		}
	}
	...
	public static String getResponse(String request) throws SystemException {
		...
		if (StringUtils.isBlank(request)) {
			logger.info("Request is empty for Payten status enquiry");
			return null;
		}
		...
		try {
			...
			hostUrl = PropertiesManager.propertiesMap.get(Constants.PAYTEN_STATUS_ENQ_URL);
			...
			logger.info("Status Enquiry Request to Payten : TxnType = " + fields.get(FieldType.TXNTYPE.getName()) + " "
			...
			try (BufferedReader in = new BufferedReader(new InputStreamReader(httpClient.getInputStream()))) {
				...
				if (StringUtils.isBlank(response)) {
					...
					logger.info("Response received for Payten status enq " + response.toString());
			...
			logger.error("Exception in Payten Status Enquiry ", e);
			return null;
		}
	...
	public void updateFields(Fields fields, String jsonResponse) {
		 ...
		 * 
		 * pay10ResultType resultInstance =
		 * pay10ResultType.getInstanceFromName(respCode);
		 * 
	...
	public Transaction toTransaction(String jsonResponse, String txnType) {
		 ...
		 * logger.info("Empty response received for pay10 Status Enquiry");
		 * return transaction; }
		 * 
