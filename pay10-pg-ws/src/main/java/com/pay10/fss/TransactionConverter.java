...
public class TransactionConverter {
	...
	public String saleRequest(Fields fields, Transaction transaction) throws SystemException {
		...
		String responseUrl = returnUrlCustomizer.customizeReturnUrl(fields, PropertiesManager.propertiesMap.get(Constants.RESPONSE_URL));
		...
		getElement(MEMBER, fields.get(FieldType.CARD_HOLDER_NAME.getName()), xml);
		...
		try {
			encryptedRequest = fssUtil.encryptRequest(xml.toString(), key);
	...
	public String saleRequestNb(Fields fields, Transaction transaction) throws SystemException {
		...
		String language = PropertiesManager.propertiesMap.get(Constants.LANGUAGE);
		String key = transaction.getTxnKey();
		String bankCode = FssBankCode.getBankCode(fields.get(FieldType.MOP_TYPE.getName()));
	...
	public void logRequest(String requestMessage, Fields fields) {
		log("Request message to FSS: TxnType = " + fields.get(FieldType.TXNTYPE.getName()) + " " + "Txn id = "
