...
@Service("freeChargeProcessor")
public class FreeChargeProcessor implements Processor {

	@Autowired
	private FreeChargeIntegrator freeChargeIntegrator;
	...
	public void process(Fields fields) throws SystemException {
		...
		if ((fields.get(FieldType.TXNTYPE.getName()).equals(TransactionType.NEWORDER.getName())
			...
			// New Order Transactions are not processed by FreeCharge
			return;
		}

		if (!fields.get(FieldType.ACQUIRER_TYPE.getName()).equals(AcquirerType.FREECHARGE.getCode())) {
			return;
		}
		...
		freeChargeIntegrator.process(fields);
