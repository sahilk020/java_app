...
@Service
public class AgreepayValidator implements Validator{
	private static Logger logger = LoggerFactory.getLogger(AgreepayValidator.class.getName());
	private static final String dummyCVV = "111";
    private static final String expDate = "102055";
    private static final String cardName = "Sonu";
	@Override
	...
	public void validateMandatoryFields(Fields fields, Collection<String> mandatoryEnrollmentFields) throws SystemException{
		...
		//Added by sonu chaudhari..Add payment type specific mandatory fields
		if(fields.get(FieldType.PAYMENT_TYPE.getName()).equals(PaymentType.NET_BANKING.getCode())) {
			logger.info("Inside NetBanking Block For Agreepay");
			return;
		}
		
		//Added by sonu chaudhari..Add payment type specific mandatory fields
		if(fields.get(FieldType.PAYMENT_TYPE.getName()).equals(PaymentType.WALLET.getCode())) {
			logger.info("Inside Wallet Block For Agreepay");
			return;
		}
		for(String key : mandatoryEnrollmentFields){
			...
			if(null == fields.get(key)){
				...
				// Added By Sonu Chaudhari (Testing purpose)
				else if(FieldType.CUST_NAME.getName().equals(key) && mopType.equals(MopType.MASTERCARD.getCode())){
					fields.put(FieldType.CUST_NAME.getName(), cardName);
					continue;
				}
