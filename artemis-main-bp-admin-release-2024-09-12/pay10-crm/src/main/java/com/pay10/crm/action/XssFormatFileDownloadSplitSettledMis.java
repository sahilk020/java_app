...
public class XssFormatFileDownloadSplitSettledMis extends AbstractSecureAction{
	...
	public String execute() {
		...
		logger.info("Inside Split Settled MIS Report Download");	
		List<MISReportObject> transactionList = new ArrayList<MISReportObject>();
		sessionUser = (User) sessionMap.get(Constants.USER.getValue());
		transactionList = srq.settlementReportDownloadSplitSettled(merchant, name, currency, dateFrom, dateTo,sessionUser);

		logger.info("List generated successfully for Split Settled MIS Report , size = "+transactionList.size());
		SXSSFWorkbook wb = new SXSSFWorkbook(100);
		Row row;
		int rownum = 1;
		// Create a blank sheet
		Sheet sheet = wb.createSheet("Split Settled MIS Report");
		...
		row.createCell(9).setCellValue("TDR Amount");
		row.createCell(10).setCellValue("GST on TDR Amount");
		//add new column TDR Amount below gross transaction amount
		//add new column GST on TDR Amount below TDR amount
		//row.createCell(9).setCellValue("Total Aggregator Commission Amt Payable(Including GST)");//remove
		//row.createCell(10).setCellValue("Total Acquirer Commission Amt Payable(Including GST)");//remove
		row.createCell(11).setCellValue("Total Amt Payable to Merchant A/c");
		//row.createCell(12).setCellValue("Total Payout from Nodal Account");//remove
		//row.createCell(13).setCellValue("BankName_Receive_Funds");//remove
		//row.createCell(14).setCellValue("Nodal a/c no");//remove
		//row.createCell(15).setCellValue("Aggregator name");//remove
		//row.createCell(16).setCellValue("Acquirer Name");//remove
		//row.createCell(17).setCellValue("Refund Flag");//remove
		row.createCell(12).setCellValue("Payments Type");
		row.createCell(13).setCellValue("MOP Type");
		//row.createCell(20).setCellValue("Account Number");//remove
		//row.createCell(21).setCellValue("IFSC Code");//remove
		//row.createCell(22).setCellValue("A/C Holder Name");//remove
		//row.createCell(23).setCellValue("Settlement Cycle");//remove
		//add new column UTR Number
		row.createCell(14).setCellValue("UTR NO");
		row.createCell(15).setCellValue("Account Number");
		...
		if (transactionList.size() < 800000) {
			for (MISReportObject transactionSearch : transactionList) {
				...
				Object[] objArr = transactionSearch.myCsvMethodSplitSettled();
		...
		try {
			...
			filename = "Split_Settled_Report" + df.format(new Date()) + FILE_EXTENSION;
			...
			logger.info("File generated successfully for Split Settled MIS Report");
