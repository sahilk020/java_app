 ...
 *
 */
public class RouterRuleRemovePayoutAction extends AbstractSecureAction {

	private static Logger logger = LoggerFactory.getLogger(RouterRuleRemovePayoutAction.class.getName());
	...
	public String deleteRules() {
		...
		try {
			...
			RouterRulePayout routerRule = routerRuleDao.findRule(id);
			RouterRulePayout pendingRouterRule = null;
			...
			List<RouterConfigurationPayout> pendingRouterConfigurationList = new ArrayList<RouterConfigurationPayout>();
			if (routerRule != null) {
			...
			if (pendingRouterConfigurationList.size() > 0) {
				...
				for (RouterConfigurationPayout routerConfiguration : pendingRouterConfigurationList) {
	...
	public void deleteRouterRuleConfiguration(RouterRulePayout routerRule) {
		...
		try {
			...
			String identifier = routerRule.getMerchant()  + routerRule.getChannel()
					+ routerRule.getCurrency() + routerRule.getTransactionType()
			...
			for (String currentSlab : slabArray) {
				...
				List<RouterConfigurationPayout> routerConfigurationListBySlab = routerConfigurationDao
