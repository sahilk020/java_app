		...
		try {
			...
			if (sessionUser.getUserType().equals(UserType.MERCHANT) ) {
				...
				recordsFiltered = recordsTotal;
					...
					BigInteger bigInt = BigInteger.valueOf(txnReports.newSearchPaymentCount(transactionId, orderId, customerEmail, customerPhone, paymentType, 
							status, currency, transactionType, mopType, "ALL", merchant.getPayId(), dateFrom, dateTo, sessionUser, start, length,tenantId,ipAddress,totalAmount,rrn,channelName,minAmount,maxAmount,columnName,logicalCondition,searchText,newDespositor,columnName1,logicalCondition1,searchText1,columnName2,logicalCondition2,searchText2));//,ipAddress,totalAmount));
					...
					aaData = encoder.encodeTransactionSearchObjNew(txnReports.newSearchPayment(transactionId, orderId,
							...
							ipAddress, totalAmount,rrn,channelName,minAmount,maxAmount,columnName,logicalCondition,searchText,newDespositor,columnName1,logicalCondition1,searchText1,columnName2,logicalCondition2,searchText2));
					logger.info("startTime, ==========TransactionSearchAction========================= = " + startTime);
					logger.info("endTime, ===========TransactionSearchAction======================== = " + endTime);
					recordsFiltered = recordsTotal;
					...
					BigInteger bigInt = BigInteger.valueOf(txnReports.newSearchPaymentCount(transactionId, orderId, customerEmail, customerPhone, paymentType, 
							status, currency, transactionType, mopType, "ALL", merchantPayId, dateFrom, dateTo, sessionUser, start, length,tenantId,ipAddress,totalAmount,rrn,channelName,minAmount,maxAmount,columnName,logicalCondition,searchText,newDespositor,columnName1,logicalCondition1,searchText1,columnName2,logicalCondition2,searchText2));//,ipAddress,totalAmount));
					setRecordsTotal(bigInt);
					if (getLength() == -1) {
						setLength(getRecordsTotal().intValue());
					}
