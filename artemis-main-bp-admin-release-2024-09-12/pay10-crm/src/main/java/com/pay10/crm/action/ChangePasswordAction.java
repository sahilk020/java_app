...
public class ChangePasswordAction extends AbstractSecureAction {
	...
	public String execute() {
		...
		try {
			...
			responseObject = changeUserPassword.changePassword(sessionUser.getEmailId(), oldPassword, newPassword);
			...
			// store the details of updated user as we can not store the password in db for
			// audit trail
			AuditTrail auditTrail = new AuditTrail(mapper.writeValueAsString(sessionUser), null,
					actionMessagesByAction.get("changePassword"));
	...
	public void validate() {
		...
		if ((validator.validateBlankField(getOldPassword()))) {
			...
			addFieldError(CrmFieldConstants.OLD_PASSWORD.getValue(), ErrorType.EMPTY_FIELD.getResponseMessage());
