		...
		try {
			...
			if (user.getUserType().equals(UserType.RESELLER)) {
				if (getEmailId().equals(CrmFieldConstants.ALL_MERCHANTS.getValue())) {
					setStatistics(barChartQuery.statisticsSummary(getEmailId(), getCurrency(),startDate,endDate,userType,user));
				} else {
					User emailUser = userDao.findPayIdByEmail(getEmailId());
				
					setStatistics(barChartQuery.statisticsSummary(emailUser.getPayId(), getCurrency(),startDate,endDate,userType,user));
				}
				return SUCCESS;
				...
				if (user.getUserType().equals(UserType.MERCHANT) || user.getUserType().equals(UserType.SUBUSER)) {
					setStatistics(barChartQuery.statisticsSummary(user.getPayId(), getCurrency(),startDate,endDate,userType,user));
				} else {
					if (getEmailId().equals(CrmFieldConstants.ALL_MERCHANTS.getValue())) {
						setStatistics(barChartQuery.statisticsSummary(getEmailId(), getCurrency(),startDate,endDate,userType,user));
					} else {
	...
	public String refund() {
		try {
			...
			if (user.getUserType().equals(UserType.RESELLER)) {
				...
				if (getEmailId().equals(CrmFieldConstants.ALL_MERCHANTS.getValue())) {
					setStatistics(barChartQuery.statisticsSummaryRefund(getEmailId(), getCurrency(), startDate, endDate,
							userType, user));
				} else {
					User emailUser = userDao.findPayIdByEmail(getEmailId());

					setStatistics(barChartQuery.statisticsSummaryRefund(emailUser.getPayId(), getCurrency(), startDate,
							endDate, userType, user));
				}
				return SUCCESS;
			}
			else {
