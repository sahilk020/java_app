...
public class DashboardReport extends AbstractSecureAction {
	...
	private static Logger logger = LoggerFactory.getLogger(DashboardReport.class.getName());
	...
	public String execute() {
		...
		try {
			...
			logger.info("Search Type : "+t+"\tstart : "+start+"\tlength : "+length  );
			
			BigInteger bigInt = BigInteger.valueOf(dashboardService.dashboardReportCount(t, start, length));//,ipAddress,totalAmount));
			setRecordsTotal(bigInt);
			if (getLength() == -1) {
				setLength(getRecordsTotal().intValue());
			}
			aaData = encoder.encodeTransactionSearchObjNew(dashboardService.dashboardReport(t, start, length));
