...
public class SettledTransactionSearchAction extends AbstractSecureAction {
	...
	public String execute() {
		...
		setStatus(StatusType.SETTLED_SETTLE.getName());
		...
		try {
			...
			if (sessionUser.getUserType().equals(UserType.RESELLER)) {
				totalCount = txnReports.searchPaymentCount(getTransactionId(), getOrderId(), getCustomerEmail(),
						...
						getDateFrom(), getDateTo(), sessionUser,getPhoneNo(),getMopType(),"",channel);
				...
				aaData = encoder.encodeTransactionSearchObj(txnReports.searchPayment(getTransactionId(), getOrderId(),
						...
						getTransactionType(), getDateFrom(), getDateTo(), sessionUser, getStart(), getLength(),getPhoneNo(),getMopType(),"",channel));
