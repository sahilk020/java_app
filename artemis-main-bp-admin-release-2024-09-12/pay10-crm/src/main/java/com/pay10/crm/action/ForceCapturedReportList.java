...
public class ForceCapturedReportList extends AbstractSecureAction {
    /**
	 * 
	 */
    ...
    public String execute() {
        ...
        if ((!StringUtils.isBlank(dateFrom) && !StringUtils.isBlank(dateTo))) {
            BigInteger bigInt = BigInteger.valueOf(transactionStatus.getForceCaptureReportCount(merchant, status,
                    acquirer, dateFrom, dateTo));// ,ipAddress,totalAmount));
        	...
        	aaData = encoder.encodeTransactionSearchObjNew(transactionStatus.getForceCapturedReport(merchant, status, acquirer, dateFrom,
                    dateTo,start,length));
