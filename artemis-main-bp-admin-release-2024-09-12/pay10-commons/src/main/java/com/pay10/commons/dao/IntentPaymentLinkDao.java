...
@Component
public class IntentPaymentLinkDao {
	private static Logger logger = LoggerFactory.getLogger(IntentPaymentLinkDao.class.getName());
...
//					PropertiesManager.propertiesMap.get(PREFIX + Constants.INVOICE_COLLECTION_NAME.getValue()));
		MongoCollection<Document> collection = dbIns.getCollection("intentPaymentLink");
	...
	public PaymentLink update(PaymentLink paymentLink) {
		...
		try {
			...
			logger.info("Intent Payment Link updated successfully : " + paymentLink.getPaymentLinkId());
		} catch (Exception e) {
			logger.info("Failed to update intent payment link : " + paymentLink.getPaymentLinkId());
