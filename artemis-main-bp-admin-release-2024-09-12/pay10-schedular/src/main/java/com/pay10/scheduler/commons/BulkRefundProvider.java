...
@Service
public class BulkRefundProvider {
	...
	private static final Logger logger = LoggerFactory.getLogger(BulkRefundProvider.class);

	public void getBulkRefundTrnsaction() throws SystemException {
		String endTime = getCurrentDate();
		String bulkRefundConfig = configurationProvider.getBulkRefundConfig();

		String startTime = getStartDate(bulkRefundConfig);

		String bulkRefundUrl = configurationProvider.getBulkRefundUrl();

		logger.info("Scheduler bulk refund Start Time = " + startTime);
		logger.info("Scheduler bulk refund config Time = " + bulkRefundConfig);

		logger.info("Scheduler bulk refund End  Time = " + endTime);

		BasicDBObject dateTimeQuery = new BasicDBObject();
		BasicDBObject txnTypQuery = new BasicDBObject();

		dateTimeQuery.put("createDate",
		...
		logger.info("Query to get data for  bulk Refund = " + finalquery);
		MongoDatabase dbIns = mongoInstance.getDB();
		MongoCollection<Document> collection = dbIns.getCollection("bulkRefundEntity");

		BasicDBObject match = new BasicDBObject("$match", finalquery);
		logger.info("Query to get data forbulk Refund " + match);
		...
		while (cursor.hasNext()) {
			...
			if (null != dbobj.get("amount") && !dbobj.getString("amount").equalsIgnoreCase("0")) {
				fields.put(FieldType.AMOUNT.getName(), (dbobj.getString("amount")).replace(".0", ""));
			}
			...
			fields.put(FieldType.REFUND_FLAG.getName(), dbobj.getString("refund_Flag"));
			...
			logger.info("fields in bulkrefund " + fields + bulkRefundUrl);
			Map<String, String> response = null;

			response = bulkRefundCall(fields, bulkRefundUrl);
			logger.info("fields in bulkrefund " + response.get("STATUS"));

			logger.info("fields in bulkrefund " + response);
