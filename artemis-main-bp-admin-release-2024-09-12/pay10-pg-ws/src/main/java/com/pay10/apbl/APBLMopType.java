...
public enum APBLMopType {
	...
	private APBLMopType(String bankName, String code, String bankCode){
	...
	public static APBLMopType getInstance(String name){
		APBLMopType[] mopTypes = APBLMopType.values();
		for(APBLMopType mopType : mopTypes){
	...
	public static List<APBLMopType> getGetMopsFromSystemProp(String mopsList){

		List<String> mopStringList= (List<String>) Helper.parseFields(PropertiesManager.propertiesMap.get(mopsList));

		List<APBLMopType> mops = new ArrayList<APBLMopType>();

		for(String mopCode:mopStringList){
			APBLMopType mop = getmop(mopCode);
			mops.add(mop);
		}
		return mops;
	}
	...
	public static String getmopName(String mopCode){
		APBLMopType mopType = APBLMopType.getmop(mopCode);		
		if(mopType == null) {
			return "";
		}
	...
	public static String getBankCode(String code){
		APBLMopType mopType = APBLMopType.getmop(code);		
		if(mopType == null) {
			return "";
		}
	...
	public static APBLMopType getmop(String mopCode){
		APBLMopType mopObj = null;
		if(null!=mopCode){
			for(APBLMopType mop:APBLMopType.values()){
	...
	public static APBLMopType getInstanceIgnoreCase(String name){
		APBLMopType[] mopTypes = APBLMopType.values();
		for(APBLMopType mopType : mopTypes){
