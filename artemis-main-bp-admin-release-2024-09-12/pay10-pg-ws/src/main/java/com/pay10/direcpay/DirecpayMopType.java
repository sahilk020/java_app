...
public enum DirecpayMopType {
	...
	//DIRECPAY LIVE
		...
		PUNJAB_NATIONAL_BANK ("Punjab National Bank", "1002","1071"),
	...
	private DirecpayMopType(String bankName, String code, String bankCode){
	...
	public static DirecpayMopType getInstance(String name){
		DirecpayMopType[] mopTypes = DirecpayMopType.values();
		for(DirecpayMopType mopType : mopTypes){
	...
	public static List<DirecpayMopType> getGetMopsFromSystemProp(String mopsList){

		List<String> mopStringList= (List<String>) Helper.parseFields(PropertiesManager.propertiesMap.get(mopsList));

		List<DirecpayMopType> mops = new ArrayList<DirecpayMopType>();

		for(String mopCode:mopStringList){
			DirecpayMopType mop = getmop(mopCode);
			mops.add(mop);
		}
		return mops;
	}
	...
	public static String getmopName(String mopCode){
		DirecpayMopType mopType = DirecpayMopType.getmop(mopCode);		
		if(mopType == null) {
			return "";
		}
	...
	public static String getBankCode(String code){
		DirecpayMopType mopType = DirecpayMopType.getmop(code);		
		if(mopType == null) {
			return "";
		}
	...
	public static DirecpayMopType getmop(String mopCode){
		DirecpayMopType mopObj = null;
		if(null!=mopCode){
			for(DirecpayMopType mop:DirecpayMopType.values()){
	...
	public static DirecpayMopType getInstanceIgnoreCase(String name){
		DirecpayMopType[] mopTypes = DirecpayMopType.values();
		for(DirecpayMopType mopType : mopTypes){
