...
@Service
public class PhonePeStatusEnquiryProcessor {

	@Autowired
	@Qualifier("phonepeTransactionConverter")
	private TransactionConverter converter;

	private static Logger logger = LoggerFactory.getLogger(PhonePeStatusEnquiryProcessor.class.getName());
	...
	public void enquiryProcessor(Fields fields) {
		...
		try {
			...
			logger.error("Exception in decrypting status enquiry response for PhonePe ", e);
		}
	...
	public String statusEnquiryRequest(Fields fields) {
		...
		try {
			...
			logger.error("Exception in preparing Phonepe Status Enquiry Request", e);
			return null;
		}
	}
	...
	public static String getResponse(String request) throws SystemException {
		...
		if (StringUtils.isBlank(request)) {
			logger.info("Request is empty for Phonepe status enquiry");
			return null;
		}
		...
		try {
			...
			logger.info("Status Enquiry Request to PhonePe : TxnType = " + fields.get(FieldType.TXNTYPE.getName()) + " "
			...
			try (BufferedReader in = new BufferedReader(new InputStreamReader(httpClient.getInputStream()))) {
				...
				if (StringUtils.isBlank(response)) {
					...
					logger.info("Response received for PhonePe status enq " + response.toString());
			...
			logger.error("Exception in Phonepe Status Enquiry ", e);
			return null;
		}
	...
	public void updateFields(Fields fields, String jsonResponse) {
		...
		if (StringUtils.isNotBlank(transaction.getAmount())
			...
			errorType = ErrorType.AMOUNT_MISMATCH;
			pgTxnMsg = ErrorType.AMOUNT_MISMATCH.getResponseMessage();
		}

		else if (StringUtils.isNotBlank(transaction.getSuccess()) && StringUtils.isNotBlank(transaction.getCode())
		...
		fields.put(FieldType.PG_RESP_CODE.getName(), transaction.getPayResponseCode());
	...
	public Transaction toTransaction(String jsonResponse) {
		...
		if (StringUtils.isBlank(jsonResponse)) {
			logger.info("Empty response received for Phonepe Status Enquiry");
			return transaction;
		}
		...
		if (respObj.has("data")) {
			...
			JSONObject data = new JSONObject(respObj);
			...
			if (data.has(Constants.TRANSACTION_ID)) {
				transaction.setTransactionId(data.getString(Constants.TRANSACTION_ID));
			}
