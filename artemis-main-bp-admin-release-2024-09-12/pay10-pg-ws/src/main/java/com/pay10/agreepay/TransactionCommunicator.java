...
@Service("agreepayTransactionCommunicator")
public class TransactionCommunicator {
	...
	public void updateSaleResponse(Fields fields, String request) {
		...
		fields.put(FieldType.AGREEPAY_FINAL_REQUEST.getName(), request);
	...
	public String refundPostRequest(String request, String hostUrl) throws SystemException {
		...
		try {
			...
			logger.error("Exception in getting Refund respose for Agrrepay", e);
	...
	public String statusEnqPostRequest(String request, String hostUrl) throws SystemException {
		...
		try {
			...
			logger.info("Agreepay Status Enquiry Request " + request);
			HttpURLConnection connection = null;
			URL url;
			url = new URL(hostUrl);
			connection = (HttpURLConnection) url.openConnection();
			...
			logger.info("Response for Agreepay Status Enquiry transaction >> " + response);

		} catch (Exception e) {
			logger.error("Exception in getting Status Enquiry respose for Agreepay", e);
		}
		return response;
	}
