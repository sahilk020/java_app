...
@Service
public class SbiSaleResponseHandler {

	private static Logger logger = LoggerFactory.getLogger(SbiSaleResponseHandler.class.getName());
	...
	@Autowired
	private SbiTransformer sbiTransformer;
	...
	public Map<String, String> process(Fields fields) throws SystemException, UnsupportedEncodingException {
		...
		logger.info("Sbi double verification encrypted response "+ fields.get(FieldType.PG_REF_NUM.getName()) + " " + response);
		...
		logger.info("SBI Sale Response Process::  pgRefNo={},totalAmount={},respAmount={} ", fields.get(FieldType.PG_REF_NUM.getName()), fields.get(FieldType.TOTAL_AMOUNT.getName()), transactionResponse.getAmount());
		if(StringUtils.equalsIgnoreCase(transactionResponse.getStatus(), "Pending")) {
			logger.info("SBI Sale Response Received For Pending ::  pgRefNo={}", fields.get(FieldType.PG_REF_NUM.getName()));
	...
	public String transactStatus(String encRequest, Fields fields) throws SystemException {
		...
		logger.info("SBI Acquirer MerchantId pgRefNum "+fields.get(FieldType.PG_REF_NUM.getName()) + " MerchantId " +merchantId );
	...
	public String getTxnKey(Fields fields) throws SystemException {
		...
		logger.info("SBI getTxnKey :: "+ fields.get(FieldType.PG_REF_NUM.getName()) +", "+fields.getFieldsAsString());
		User user = userDao.findPayId(fields.get(FieldType.PAY_ID.getName()));
		Account account = null;
		Set<Account> accounts = user.getAccounts();

		//logger.info("Accounts Info For SBI Acquirers : "+ fields.get(FieldType.PG_REF_NUM.getName()) +", "+accounts.toString());
		...
		//logger.info("Account Info For SBI Acquirers : "+ fields.get(FieldType.PG_REF_NUM.getName()) +", "+account.toString());
	...
	public String doubleVerificationRequest(Fields fields) throws SystemException, UnsupportedEncodingException {
		...
		logger.info("Double verification encrypted request "+ fields.get(FieldType.PG_REF_NUM.getName()) + " " + encryptedRequest);
