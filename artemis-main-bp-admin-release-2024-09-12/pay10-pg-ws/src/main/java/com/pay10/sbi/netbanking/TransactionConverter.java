...
@Service("sbiNBTransactionConverter")
public class TransactionConverter {
	...
	public String saleRequest(Fields fields, Transaction transaction) throws SystemException {
		...
		if(fields.get(FieldType.PAYMENT_TYPE.getName()).equalsIgnoreCase(PaymentType.CREDIT_CARD.getCode())
			...
			jsonRequest.addProperty(Constants.orderCurrency,Currency.getAlphabaticCode(fields.get(FieldType.CURRENCY_CODE.getName())));
		 ...
		 * req.append(checksum.toLowerCase());
		 */
		logger.info("Plain Text Request to SBI " + fields.get(FieldType.PG_REF_NUM.getName()) + ":" + req.toString());
		String encryptedRequest = sbiUtil.encrypt(req.toString());
		logger.info("Encrypted Request to SBI " + fields.get(FieldType.PG_REF_NUM.getName()) + ":" + encryptedRequest);
