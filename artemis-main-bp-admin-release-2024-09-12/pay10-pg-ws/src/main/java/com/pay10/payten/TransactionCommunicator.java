...
@Service("paytenTransactionCommunicator")
public class TransactionCommunicator {
	...
	public void updateSaleResponse(Fields fields, String request) {
		...
		fields.put(FieldType.PAY10_FINAL_REQUEST.getName(), request.toString());
	...
	public String getResponse(String request, Fields fields) throws SystemException {
		...
		try {
			...
			case CAPTURE:
				break;
			case REFUND:
				hostUrl = PropertiesManager.propertiesMap.get(Constants.PAYTEN_REFUND_URL);
				break;
			case STATUS:
			...
			logger.info("Request message to Payten : TxnType = " + fields.get(FieldType.TXNTYPE.getName()) + " "
			...
			if ((responseData = responseReader.readLine()) != null) {
				logger.info("Payten Refund Response >>> "+responseData);
			}
			...
			logger.error("Exception in Payten txn Communicator", e);
		}
		return null;
