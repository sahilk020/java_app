...
@Service
public class AxisBankCardStatusEnquiryProcessor {
	...
	private static Logger logger = LoggerFactory.getLogger(AxisBankCardStatusEnquiryProcessor.class.getName());
	...
	public String statusEnquiryRequest(Fields fields) {
		...
		String statusEnqString = fields.get(FieldType.MERCHANT_ID.getName())+"||"+fields.get(FieldType.ADF1.getName())+"||";
		String paymentString = "1||010|"+fields.get(FieldType.PG_REF_NUM.getName());
	...
	public static String getResponse(String request) throws SystemException {
		...
		try {
			...
			logger.info(
					"Status Enquiry Request message to direcpay: TxnType = " + fields.get(FieldType.TXNTYPE.getName())
			...
			logger.info("Status Enquiry mesage from direcpay: TxnType = " + fields.get(FieldType.TXNTYPE.getName())
					+ " " + "Order id = " + fields.get(FieldType.ORDER_ID.getName()) + " " + pipedResponse);
	...
	public void updateFields(Fields fields, String pipedResponse) {
...

		{
			...
			if ((StringUtils.isNotBlank(transaction.getResponse_Err_Code()))
				...
				AxisBankCardResultType resultInstance = AxisBankCardResultType
						.getInstanceFromName(transaction.getResponse_Err_Code());
