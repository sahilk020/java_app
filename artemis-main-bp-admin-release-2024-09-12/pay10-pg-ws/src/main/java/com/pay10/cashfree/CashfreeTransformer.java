...
public class CashfreeTransformer {
	private static Logger logger = LoggerFactory.getLogger(CashfreeTransformer.class.getName());
	...
	public void updateResponse(Fields fields) {
    	...
    	//ErrorMappingDTO errorMappingDTO = errorMappingService.getErrorMappingByAcqCode(transaction.getTxStatus(), "CASHFREE");
		ErrorMappingDTO errorMappingDTO = new ErrorMappingDAOImpl().getErrorMappingByAcqCode(transaction.getTxStatus(), "CASHFREE");
	...
	public void updateRefundResponse(Fields fields) {
		...
		if ((StringUtils.isNotBlank(transaction.getStatus()))
			...
			pgTxnMsg = ErrorType.PENDING.getResponseMessage();
				...
				&& ((transaction.getStatus()).equalsIgnoreCase("CANCELLED"))) {
			status = StatusType.CANCELLED.getName();
			errorType = ErrorType.CANCELLED;
			pgTxnMsg = ErrorType.CANCELLED.getResponseMessage();
		} else if ((StringUtils.isNotBlank(transaction.getStatus()))
				&& ((transaction.getStatus()).equalsIgnoreCase("ONHOLD"))) {
			status = StatusType.ONHOLD.getName();
			errorType = ErrorType.ONHOLD;
			pgTxnMsg = ErrorType.ONHOLD.getResponseMessage();
		} else {
	...
	public void updateRefundResponseOld(Fields fields) {
