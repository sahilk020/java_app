 ...
 *
 */
public class PinelabsResponseAction extends AbstractSecureAction implements ServletRequestAware {

	private static Logger logger = LoggerFactory.getLogger(PinelabsResponseAction.class.getName());
	...
	public String execute() {
		try {
			...
			for (Entry<String, String> entry : requestMap.entrySet()) {
				...
				if (entry.getKey().equalsIgnoreCase("unique_merchant_txn_id")) {
					pgRef = entry.getValue();
				}
				if (entry.getKey().equalsIgnoreCase("merchant_access_code")) {
					access_code = entry.getValue();// in case of send redirect, session not there
				}

			}

			logger.info("Response received from Pinelabs: " + resJson.toString());
			...
			logger.info(merchantId + "Response received from Pinelabs: " + txnKey + " -----" + pgRef);
				...
				if (StringUtils.isNotBlank(pgRef)) {
					...
					if (StringUtils.isNotBlank(paramMap.get(FieldType.IS_MERCHANT_HOSTED.getName()))) {
						logger.info("IS_MERCHANT_HOSTED flag found for ORDER ID "
								+ paramMap.get(FieldType.ORDER_ID.getName()) + " in PINELABS");
			...
			fields.put(FieldType.PINELABS_RESPONSE_FIELD.getName(), resJson.toString());
			fields.put(FieldType.TXN_KEY.getName(), txnKey);
			fields.put(FieldType.MERCHANT_ID.getName(), merchantId);
			fields.logAllFields("Pinelabs Response Recieved :");

			
			String adf1 = (String) sessionMap.get(FieldType.ADF1.getName());
			fields.logAllFields("Updated 3DS Recieved Map TxnType = " + fields.get(FieldType.TXNTYPE.getName()) + " "
					+ "Txn id = " + fields.get(FieldType.TXN_ID.getName()) + "m access code : "
					+ sessionMap.get(FieldType.ADF1.getName()));
			fields.put(FieldType.ACQUIRER_TYPE.getName(), AcquirerType.PINELABS.getCode());
			...
			if (StringUtils.isBlank(adf1)) {
				fields.put((FieldType.ADF1.getName()), access_code);
			} else {
				fields.put((FieldType.ADF1.getName()), (String) sessionMap.get(FieldType.ADF1.getName()));
			}
			Map<String, String> response = transactionControllerServiceProvider.transact(fields,
					Constants.TXN_WS_PINELABS_PROCESSOR.getValue());
		...
		if (accounts == null || accounts.size() == 0) {
			...
			for (Account accountThis : accounts) {
				if (accountThis.getAcquirerName().equalsIgnoreCase(
						AcquirerType.getInstancefromCode(AcquirerType.PINELABS.getCode()).getName())) {
					account = accountThis;
					break;
				}
			}
