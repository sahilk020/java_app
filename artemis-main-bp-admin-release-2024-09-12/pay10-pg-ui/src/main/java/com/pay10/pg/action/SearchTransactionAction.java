...
public class SearchTransactionAction extends AbstractSecureAction {
	...
	public String execute() {
		...
		try {
			...
			if (responseMap == null) {
				...
				fields.logAllFields("final upi string ");
				...
				String enData = merchantHostedUtils.encryptMerchantResponse(fields);

				setPayId(fields.get(FieldType.PAY_ID.getName()));
				String isMerchantHosted = (String) sessionMap.get(FieldType.IS_MERCHANT_HOSTED.getName());
				responseFields.put(Constants.ENC_DATA.getValue(), enData);
				// E-Ticketing
				...
				// Added BY Sweety
				if (fields.get(FieldType.STATUS.getName()) != null) {
					if (StatusType.CAPTURED.getName().equalsIgnoreCase(fields.get(FieldType.STATUS.getName()))) {
						...
						logger.info("ServiceTransactionAction  ,STATUS >>>>=" + fields.get(FieldType.STATUS.getName()));
