 ...
 */
public class FileLineMapper<T> implements LineMapper<Fields>, InitializingBean {
	
	private static final Logger logger = LoggerFactory.getLogger(FileLineMapper.class);
	...
	public Fields mapLine(String line, int lineNumber) throws Exception {
		...
		for (String fieldName : fileParameter) {
			...
			String val = fieldSet.readString(fieldName).replace("\'", "").replaceAll("[^\\p{ASCII}]", "");
			if(fieldName.equals(FieldType.PG_REF_NUM.getName())){

				MDC.put("PG_REF_NUM",val);
			}
