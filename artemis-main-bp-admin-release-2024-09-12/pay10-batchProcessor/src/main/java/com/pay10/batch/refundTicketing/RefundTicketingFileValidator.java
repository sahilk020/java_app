...
/** Performing refund reco Ticketing file data validations. */
@Service
public class RefundTicketingFileValidator extends FileValidator {

	private static final Logger logger = LoggerFactory.getLogger(RefundTicketingFileValidator.class);
	...
	private boolean readPreviousValuesFromDB(Fields fields) throws DatabaseException
	{
		...
		if (Constants.RECO_TRUE.getValue().equalsIgnoreCase(fields.get(FieldType.IS_REFUND_RECO_RECONCILED.getName()))) {
			logger.info("There is already a transaction with RECO and SETTLED state in DB for this REFUND transaction with Pg_Ref_Num: " + pg_ref_num);
			return false;
		}
		// Refund Capture present
		if (isTransactionValid(fields)) {
			if (Constants.RECO_TRUE.getValue().equalsIgnoreCase(fields.get(FieldType.IS_SALE_CAPTURED.getName()))) {
				...
				if ((isBankSettlementIdValid(fields, fields.get(FieldType.RECO_ORDER_ID.getName())))
						&& (isAmountValid(fields, fields.get(FieldType.REFUND_AMOUNT.getName())))
						&& (isSaleTxnValid(fields)))
