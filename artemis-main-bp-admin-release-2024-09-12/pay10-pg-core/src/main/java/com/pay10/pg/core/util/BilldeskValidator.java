...
public class BilldeskValidator implements Validator{
	...
	public void validateMandatoryFields(Fields fields, Collection<String> mandatoryEnrollmentFields) throws SystemException{
		...
		/*
		 * for(String key : mandatoryEnrollmentFields){ String cardBin =
		 * fields.get(FieldType.CARD_NUMBER.getName()).replace(" ", "").replace(",",
		 * "").substring(0, 4); if(null == fields.get(key)){ if
		 * (FieldType.CVV.getName().equals(key) &&
		 * mopType.equals(MopType.MAESTRO.getCode())){
		 * fields.put(FieldType.CVV.getName(),dummyCVV); continue; }else
		 * if(FieldType.CARD_EXP_DT.getName().equals(key) &&
		 * mopType.equals(MopType.MAESTRO.getCode())){
		 * fields.put(FieldType.CARD_EXP_DT.getName(), expDate); continue; }else
		 * if((cardBin.equals("5018") || cardBin.equals("5020") ||
		 * cardBin.equals("5038") || cardBin.equals("5612") || cardBin.equals("5893") ||
		 * cardBin.equals("6304") || cardBin.equals("6759") || cardBin.equals("6220") ||
		 * cardBin.equals("0604")|| cardBin.equals("6390") || cardBin.equals("6799"))){
		 * fields.put(FieldType.CARD_EXP_DT.getName(), expDate);
		 * fields.put(FieldType.MOP_TYPE.getName(), MopType.MAESTRO.getCode());
		 * continue; }
		 * 
		 * throw new SystemException(ErrorType.VALIDATION_FAILED, key +
		 * " is a required field"); }//if }
		 *///for
	}
}
