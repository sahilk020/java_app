...
@Service
public class SbiCardValidator implements Validator {
	private static Logger logger = LoggerFactory.getLogger(SbiCardValidator.class.getName());
	...
	public void validateMandatoryFields(Fields fields, Collection<String> mandatoryEnrollmentFields)
		...
		for (String key : mandatoryEnrollmentFields) {
			String cardBin = "";
			if (StringUtils.isNotBlank(fields.get(FieldType.TOKEN_ID.getName()))
					&& !StringUtils.equalsIgnoreCase(key, FieldType.CVV.getName())) {
				continue;
			} else if (StringUtils.isBlank(fields.get(FieldType.TOKEN_ID.getName()))) {
				cardBin = fields.get(FieldType.CARD_NUMBER.getName()).replace(" ", "").replace(",", "").substring(0, 4);
			}
