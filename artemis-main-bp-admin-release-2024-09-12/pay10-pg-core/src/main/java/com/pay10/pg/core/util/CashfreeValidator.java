...
public class CashfreeValidator implements Validator{
	private static Logger logger = LoggerFactory.getLogger(CashfreeValidator.class.getName());
	...
	public void validateMandatoryFields(Fields fields, Collection<String> mandatoryEnrollmentFields) throws SystemException{
		...
		if(fields.get(FieldType.PAYMENT_TYPE.getName()).equals(PaymentType.UPI.getCode())) {
			logger.info("skip Validate Mandatory Fields For CashFreee UPI");
			return;
		}
		
		//add by sonu chaudhari for s2s 
		if(fields.get(FieldType.PAYMENT_TYPE.getName()).equals(PaymentType.NET_BANKING.getCode())) {
			logger.info("skip Validate Mandatory Fields For CashFreee NET_BANKING");
			return;
		}
		
		if(fields.get(FieldType.PAYMENT_TYPE.getName()).equals(PaymentType.WALLET.getCode())) {
			logger.info("skip Validate Mandatory Fields For CashFreee Wallet");
			return;
		}
		...
		for(String key : mandatoryEnrollmentFields){
			...
			if(null == fields.get(key)){
				...
				// code added by Sonu Chaudhari (Testing purpose)
