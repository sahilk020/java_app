...
import com.pay10.commons.dao.SummaryTransactionDataDao;
...
@Service
public class UpdateCaptureDataService {
	...
	@Autowired
	private SummaryTransactionDataDao summaryTransactionDataDao;
	...
	private static Logger logger = LoggerFactory.getLogger(UpdateCaptureDataService.class.getName());
	private static final String prefix = "MONGO_DB_";

	public void updateData(String fromDate, String toDate) {
		...
		try {
			...
			try {
				...
				if (surchargeDao.findAllSurchargeByDate(dateFrom, dateTo) == null) {
					logger.info("No surcharge data found");
				}
				else {
					surchargeList = surchargeDao.findAllSurchargeByDate(dateFrom, dateTo);
				}
			...
			logger.info("final query processing for capture data upload = " + finalquery);
			...
			logger.info("final query processed for capture data upload = " + finalquery);
			while (cursor.hasNext()) {
				Document doc = cursor.next();
				
					
					if (StringUtils.isNotBlank(doc.getString(FieldType.POST_SETTLED_FLAG.getName()))) {
						if (doc.getString(FieldType.POST_SETTLED_FLAG.getName()).equalsIgnoreCase("Y")) {
							continue;
						}
					}
					...
					settledTransactionData.setCaptureDate(doc.getString(FieldType.CREATE_DATE.getName()));
					...
					capturedTransactionDataList.add(settledTransactionData);
				}
			...
			logger.info("Found "+capturedTransactionDataList.size()+" captured transactions in iPayTransactions for date range "+fromDate+" TO "+toDate);

			summaryTransactionDataDao.insertTransaction(capturedTransactionDataList);
			
			logger.info("Added "+capturedTransactionDataList.size()+" captured transactions in iPayTransactions for date range "+fromDate+" TO "+toDate);
		} catch (Exception e) {
	...
	public void updateSettlementData(String fromDate, String toDate) {

		logger.info("Updating Post settled captured transactions");
		try {
			Map<String, User> userMap = new HashMap<String, User>();
			Map<String, List<ServiceTax>> serviceTaxMap = new HashMap<String, List<ServiceTax>>();
			...
			saleConditionList.add(new BasicDBObject(FieldType.POST_SETTLED_FLAG.getName(), Constants.Y.getValue()));
			...
			logger.info("final query processing for Post settled capture data upload = " + finalquery);
			MongoDatabase dbIns = mongoInstance.getDB();
			MongoCollection<Document> coll = dbIns
					.getCollection(PropertiesManager.propertiesMap.get(prefix + Constants.COLLECTION_NAME.getValue()));
			BasicDBObject match = new BasicDBObject("$match", finalquery);
			...
			output.allowDiskUse(true);
			MongoCursor<Document> cursor = output.iterator();
			logger.info("final query processed for Post settled capture data upload = " + finalquery);
			
			while (cursor.hasNext()) {
				Document docSettled = cursor.next();

				List<BasicDBObject> captureConditionList = new ArrayList<BasicDBObject>();
				captureConditionList.add(new BasicDBObject(FieldType.PG_REF_NUM.getName(),
						docSettled.getString(FieldType.PG_REF_NUM.getName())));
				captureConditionList.add(new BasicDBObject(FieldType.ACQ_ID.getName(),
						docSettled.getString(FieldType.ACQ_ID.getName())));
				captureConditionList.add(new BasicDBObject(FieldType.STATUS.getName(),
						StatusType.CAPTURED.getName()));
				captureConditionList.add(new BasicDBObject(FieldType.POST_SETTLED_FLAG.getName(),
						"Y"));
				
				BasicDBObject searchCollQuery = new BasicDBObject("$and", captureConditionList);
				FindIterable<Document> docIterator = coll.find(searchCollQuery);
				...
				while (mongoCursor.hasNext()) {
					...
					if (!StringUtils.isBlank(doc.getString(FieldType.SURCHARGE_FLAG.getName()))
						...
						settledTransactionData.set_id(TransactionManager.getNewTransactionId());
						settledTransactionData.setTransactionId(doc.getString(FieldType.TXN_ID.getName()));
						settledTransactionData.setPgRefNum(doc.getString(FieldType.PG_REF_NUM.getName()));	
						...
						if (StringUtils.isBlank(doc.getString(FieldType.PAYMENTS_REGION.getName()))) {
							...
							settledTransactionData
									.setTransactionRegion(doc.getString(FieldType.PAYMENTS_REGION.getName()));
						}
						...
						if (StringUtils.isBlank(doc.getString(FieldType.POST_SETTLED_FLAG.getName()))) {
							...
							settledTransactionData
									.setPostSettledFlag(doc.getString(FieldType.POST_SETTLED_FLAG.getName()));
						}
						...
						if (StringUtils.isBlank(doc.getString(FieldType.CARD_HOLDER_TYPE.getName()))) {
							...
							settledTransactionData
									.setCardHolderType(doc.getString(FieldType.CARD_HOLDER_TYPE.getName()));
						}
						...
						if (totalAmount.equals(amount)) {
							...
							BigDecimal acquirerSurcharge = amount
									.multiply(bankChargePr.divide(BigDecimal.valueOf(100)));
							acquirerSurcharge = acquirerSurcharge.add(bankFixCharge).setScale(2,
			...
			logger.info("Found " + capturedTransactionDataList.size()
					+ " post settled captured transactions in iPayTransactions for date range " + fromDate + " TO " + toDate);

			summaryTransactionDataDao.insertTransaction(capturedTransactionDataList);
			
			logger.info("Added " + capturedTransactionDataList.size()
			+ " post settled captured transactions in iPayTransactions for date range " + fromDate + " TO " + toDate);
	...
	public void findDuplicates(String fromDate, String toDate) {

		logger.info("Finding duplicate transactions ");
		try {
			...
			if (StringUtils.isNotBlank(fromDate)) {
				...
				saleConditionList.add(dateQuery);
			}

			
			BasicDBObject data1 = new BasicDBObject("TXNTYPE","SALE");
			BasicDBObject data2 = new BasicDBObject("STATUS","Captured");
			
			saleConditionList.add(data1);
			saleConditionList.add(data2);

			BasicDBObject saleConditionQuery = new BasicDBObject("$and", saleConditionList);

			logger.info("final query processing for duplicate Order Id  = " + saleConditionQuery);
			MongoDatabase dbIns = mongoInstance.getDB();
			MongoCollection<Document> coll = dbIns
					.getCollection(PropertiesManager.propertiesMap.get(prefix + Constants.COLLECTION_NAME.getValue()));
			
			MongoCollection<Document> collDuplicate = dbIns
			...
			logger.info("final query processed for duplicate Order Id  = " + saleConditionQuery);
			int counter = 0;
			int totalCount = 0;
			while (cursor.hasNext()) {
				...
				if (counter == 5000 ) {
					logger.info("Checked "+ totalCount +" records ");
					counter = 0;
				}
				
				Document docSettled = cursor.next();
				
				BasicDBObject orderIdQuery = new BasicDBObject("ORDER_ID", docSettled.getString(FieldType.ORDER_ID.getName()));
				BasicDBObject txnTypeQuery = new BasicDBObject("TXNTYPE", "SALE");
				BasicDBObject statusTypeQuery = new BasicDBObject("STATUS", "Captured");
				BasicDBObject pgRefQuery = new BasicDBObject("PG_REF_NUM", docSettled.getString(FieldType.PG_REF_NUM.getName()));
				BasicDBObject acqIdQuery = new BasicDBObject("ACQ_ID", docSettled.getString(FieldType.ACQ_ID.getName()));
				List<BasicDBObject> pgRefConditionList = new ArrayList<BasicDBObject>();
				
				pgRefConditionList.add(orderIdQuery);
				pgRefConditionList.add(txnTypeQuery);
				pgRefConditionList.add(statusTypeQuery);
				pgRefConditionList.add(pgRefQuery);
				pgRefConditionList.add(acqIdQuery);
				
				BasicDBObject pgRefConditionQuery = new BasicDBObject("$and", pgRefConditionList);

				long count = coll.count(pgRefConditionQuery);
				
				if (count > 1) {
					docSettled.put("_id", TransactionManager.getNewTransactionId());
					docSettled.put(FieldType.RESPONSE_MESSAGE.getName(), "More than one Txn for this PG_REF_NUM and ACQ_ID");
					Thread.sleep(5);
					collDuplicate.insertOne(docSettled);
				}
			}
		} catch (Exception e) {
			logger.error("Exception occured " + e);
