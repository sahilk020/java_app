...
public class AnalyticsDataServiceSMS {
	...
	public AnalyticsData getTransactionCount(String fromDate, String toDate, String payId, String paymentType,
		...
		try {
			...
			String statusList = "Rejected,Timeout,Denied due to fraud,Invalid,Cancelled,Error,Failed,Captured";
			...
			double ppiSuccess = 0;
			double ppiFailed = 0;
			double ppiCancelled = 0;
			double ppiInvalid = 0;
			double ppiFraud = 0;
			double ppiDropped = 0;
			double ppiRejected = 0;
			...
			BigDecimal totalPPITxnAmount = BigDecimal.ZERO.setScale(2, RoundingMode.HALF_DOWN);
			BigDecimal totalNBTxnAmount = BigDecimal.ZERO.setScale(2, RoundingMode.HALF_DOWN);
			
			logger.info("Query started for Capture Data " + finalQuery);
			MongoCursor<Document> cursor = coll.find(finalQuery).iterator();
			logger.info("Query End for Capture Data ");
					...
					case CAPTURED:
						...
						if (!StringUtils.isBlank(dbobj.getString(FieldType.TOTAL_AMOUNT.toString()))) {
							BigDecimal txnAmount = new BigDecimal(dbobj.getString(FieldType.TOTAL_AMOUNT.toString()))
									.setScale(2, RoundingMode.HALF_DOWN);
							totalTxnAmount = totalTxnAmount.add(txnAmount).setScale(2, RoundingMode.HALF_DOWN);
							totalPPITxnAmount = totalPPITxnAmount.add(txnAmount).setScale(2, RoundingMode.HALF_DOWN);
			...
			double totalTxnSuccess = ccSuccess + dcSuccess + upSuccess+ppiSuccess+nbSuccess;
			double totalFailed = ccFailed + dcFailed + upFailed + ppiFailed+nbFailed;
			double totalRejected = ccRejected + dcRejected + upRejected + ppiRejected+nbRejected;
			...
			if (totalTxnSuccess < 1) {
				...
				double ppiTxnPercent = (ppiSuccess / totalTxnSuccess) * 100;
				...
				analyticsData.setWLTxnPercent(String.format("%.2f", ppiTxnPercent));
			...
			if (ppiSuccess + ppiFailed < 1) {
				analyticsData.setWLSuccessRate("0.00");
			} else {
				double wlSuccessRate = (ppiSuccess / (ppiSuccess + ppiFailed)) * 100;
			...
			if (totalPPITxn < 1) {
				...
				analyticsData.setTotalWLTxn(String.format("%.0f", totalPPITxn));

				double totalPPISuccessTxnPercent = (ppiSuccess / (totalPPITxn)) * 100;
				double totalPPIFailedTxnPercent = (ppiFailed / (totalPPITxn)) * 100;
				double totalPPICancelledTxnPercent = (ppiCancelled / (totalPPITxn)) * 100;
				double totalPPIInvalidTxnPercent = (ppiInvalid / (totalPPITxn)) * 100;
				double totalPPIFraudTxnPercent = (ppiFraud / (totalPPITxn)) * 100;
				double totalPPIDroppedTxnPercent = (ppiDropped / (totalPPITxn)) * 100;
				double totalPPIRejectedTxnPercent = (ppiRejected / (totalPPITxn)) * 100;

				analyticsData.setTotalWLSuccessTxnPercent(String.format("%.2f", totalPPISuccessTxnPercent));
				analyticsData.setTotalWLFailedTxnPercent(String.format("%.2f", totalPPIFailedTxnPercent));
				analyticsData.setTotalWLCancelledTxnPercent(String.format("%.2f", totalPPICancelledTxnPercent));
				analyticsData.setTotalWLInvalidTxnPercent(String.format("%.2f", totalPPIInvalidTxnPercent));
				analyticsData.setTotalWLFraudTxnPercent(String.format("%.2f", totalPPIFraudTxnPercent));
				analyticsData.setTotalWLDroppedTxnPercent(String.format("%.2f", totalPPIDroppedTxnPercent));
				analyticsData.setTotalWLRejectedTxnPercent(String.format("%.2f", totalPPIRejectedTxnPercent));
			...
			if (paymentType.equals(PaymentType.CREDIT_CARD.getCode())) {
				...
				analyticsData.setRejectedPercent(String.format("%.2f", (upRejected / totalUPTxn) * 100));
					...
					analyticsData.setCaptured(String.format("%.0f", ppiSuccess));
					analyticsData.setFailed(String.format("%.0f", ppiFailed));
					analyticsData.setCancelled(String.format("%.0f", ppiCancelled));
					analyticsData.setInvalid(String.format("%.0f", ppiInvalid));
					analyticsData.setFraud(String.format("%.0f", ppiFraud));
					analyticsData.setDropped(String.format("%.0f", ppiDropped));
					analyticsData.setRejected(String.format("%.0f", ppiRejected));

					analyticsData.setCapturedPercent(String.format("%.2f", (ppiSuccess / totalPPITxn) * 100));
					analyticsData.setFailedPercent(String.format("%.2f", (ppiFailed / totalPPITxn) * 100));
					analyticsData.setCancelledPercent(String.format("%.2f", (ppiCancelled / totalPPITxn) * 100));
					analyticsData.setInvalidPercent(String.format("%.2f", (ppiInvalid / totalPPITxn) * 100));
					analyticsData.setFraudPercent(String.format("%.2f", (ppiFraud / totalPPITxn) * 100));
					analyticsData.setDroppedPercent(String.format("%.2f", (ppiDropped / totalPPITxn) * 100));
					analyticsData.setRejectedPercent(String.format("%.2f", (ppiRejected / totalPPITxn) * 100));
			...
			analyticsData.setTotalWLTxnAmount(String.format("%.2f", totalPPITxnAmount));
			...
			analyticsData.setTotalWLCapturedCount(String.format("%.0f", ppiSuccess));
			...
			logger.error("Exception in transaction summary count service for capture data" + e);
		}
		return null;
	}
	...
	public AnalyticsData getSettledTransaction(String fromDate, String toDate, String payId,
			String paymentType, String acquirer, User user) {
	...
	try {		
		...
		List<BasicDBObject> txnConditionsList = new ArrayList<BasicDBObject>();
		List<BasicDBObject> saleCapturedConditionList = new ArrayList<BasicDBObject>();
		saleCapturedConditionList.add(new BasicDBObject(FieldType.TXNTYPE.getName(), TransactionType.RECO.getName()));
			...
			while (cursor.hasNext()) {
				...
				case DEBIT_CARD:
					...
					case CAPTURED:
						...
						if (!StringUtils.isBlank(dbobj.getString(FieldType.TOTAL_AMOUNT.toString()))) {
							BigDecimal txnAmount = new BigDecimal(dbobj.getString(FieldType.TOTAL_AMOUNT.toString())).setScale(2, RoundingMode.HALF_DOWN);
							totalTxnAmount = totalTxnAmount.add(txnAmount).setScale(2, RoundingMode.HALF_DOWN);
							totalDCTxnAmount = totalDCTxnAmount.add(txnAmount).setScale(2, RoundingMode.HALF_DOWN);
		...
		//	totalTxnAmount = totalTxnAmount.setScale(2, RoundingMode.HALF_DOWN);
			...
			double totalTxnSuccess = ccSuccess + dcSuccess + upSuccess + wlSuccess;
			double totalFailed = ccFailed + dcFailed + upFailed + wlFailed;
			...
			saleSettledConditionList.add(new BasicDBObject(FieldType.TXNTYPE.getName(), TransactionType.RECO.getName()));
			...
			deltaRefundCapturedConditionList.add(new BasicDBObject(FieldType.TXNTYPE.getName(), TransactionType.REFUND.getName()));
			deltaRefundCapturedConditionList.add(new BasicDBObject(FieldType.STATUS.getName(), StatusType.CAPTURED.getName()));
			...
			postSettleConditionList.add(new BasicDBObject(FieldType.POST_SETTLED_FLAG.getName(), Constants.Y.getValue()));
	...
	public AnalyticsData getMerchantSMSData(String fromDate, String toDate, String payId,
		...
		try {
			...
			toDate = DateCreater.formDateTimeformatCreater(toDate);*/
			...
			refundCapturedConditionList.add(new BasicDBObject(FieldType.STATUS.getName(), StatusType.CAPTURED.getName()));
			...
			BasicDBObject refundCapturedConditionQuery = new BasicDBObject("$or", refundCapturedList);
				...
				while (cursor.hasNext()) {
					...
					if (dbobj.getString(FieldType.TXNTYPE.getName()).equalsIgnoreCase(TransactionType.SALE.getName())) {
						saleCapturedAmount = saleCapturedAmount.add(new BigDecimal(dbobj.getString(FieldType.AMOUNT.getName())));
						if (dbobj.getString(FieldType.PAYMENT_TYPE.getName()).equalsIgnoreCase(PaymentType.CREDIT_CARD.getCode())) {
							ccCapturedCount ++;
							ccTotalAmt = ccTotalAmt.add(new BigDecimal(dbobj.getString(FieldType.AMOUNT.getName())));
						}
						else if (dbobj.getString(FieldType.PAYMENT_TYPE.getName()).equalsIgnoreCase(PaymentType.DEBIT_CARD.getCode())) {
							dcCapturedCount ++;		
							dcTotalAmt = dcTotalAmt.add(new BigDecimal(dbobj.getString(FieldType.AMOUNT.getName())));
						}
						else if (dbobj.getString(FieldType.PAYMENT_TYPE.getName()).equalsIgnoreCase(PaymentType.UPI.getCode())) {
							upCapturedCount ++;
							upTotalAmt = upTotalAmt.add(new BigDecimal(dbobj.getString(FieldType.AMOUNT.getName())));
						}
						else if (dbobj.getString(FieldType.PAYMENT_TYPE.getName()).equalsIgnoreCase(PaymentType.WALLET.getCode())) {
							wlCapturedCount ++;
						...
						refundCapturedAmount = refundCapturedAmount.add(new BigDecimal(dbobj.getString(FieldType.AMOUNT.getName())));
						//logger.info(" inside getMerchantSMSData for Preparing iPaycapturedData SMS in side while loop refundCapturedAmount "  +refundCapturedAmount);
					}
				...
				if ((ccCapturedCount + dcCapturedCount  + upCapturedCount) > 0) {
					...
					ccTxnPer = (Double.valueOf(ccCapturedCount) / Double.valueOf(ccCapturedCount + dcCapturedCount  + upCapturedCount)) * 100;
					dcTxnPer = (Double.valueOf(dcCapturedCount) / Double.valueOf(ccCapturedCount + dcCapturedCount  + upCapturedCount)) * 100;
					upTxnPer = (Double.valueOf(upCapturedCount) / Double.valueOf(ccCapturedCount + dcCapturedCount  + upCapturedCount)) * 100;
					wlTxnPer = (Double.valueOf(wlCapturedCount) / Double.valueOf(wlCapturedCount + wlCapturedCount  + wlCapturedCount)) * 100;
					
					ccAmtPer = (ccTotalAmt.divide(saleCapturedAmount, 2,
							RoundingMode.HALF_DOWN)).multiply(new BigDecimal("100")).setScale(2, RoundingMode.HALF_DOWN).doubleValue();
					dcAmtPer = (dcTotalAmt.divide(saleCapturedAmount, 2,
							RoundingMode.HALF_DOWN)).multiply(new BigDecimal("100")).setScale(2, RoundingMode.HALF_DOWN).doubleValue();
					upAmtPer = (upTotalAmt.divide(saleCapturedAmount, 2,
							RoundingMode.HALF_DOWN)).multiply(new BigDecimal("100")).setScale(2, RoundingMode.HALF_DOWN).doubleValue();
					wlAmtPer = (wlTotalAmt.divide(saleCapturedAmount, 2,
							RoundingMode.HALF_DOWN)).multiply(new BigDecimal("100")).setScale(2, RoundingMode.HALF_DOWN).doubleValue();
				...
				Date dateCapFrom = new SimpleDateFormat("yyyy-mm-dd hh:mm:ss", Locale.ENGLISH).parse(captureDateArray.get(0));
				Date dateCapTo = new SimpleDateFormat("yyyy-mm-dd hh:mm:ss", Locale.ENGLISH).parse(captureDateArray.get(captureDateArray.size()-1));
		...
		try {
			...
			for (String txnType : txnTypeList) {
				...
				while (cursor.hasNext()) {
					...
					Document dbobj = cursor.next();
					//logger.info("Inside , whileLoop =>, "  +dbobj.getString(FieldType.PG_REF_NUM.toString()));
				...
				if (!StringUtils.isBlank(dbobj.getString(FieldType.SURCHARGE_FLAG.toString()))) {
					...
					if (dbobj.getString(FieldType.SURCHARGE_FLAG.toString()).equals("Y")) {
							...
							for (Surcharge surchargeData : surchargeList) {
								...
								if (AcquirerType.getInstancefromName(surchargeData.getAcquirerName()).toString()
										...
										&& surchargeData.getPaymentsRegion().name()
												.equalsIgnoreCase(transactionRegion)
										&& surchargeData.getPayId().equalsIgnoreCase(dbobj.getString(FieldType.PAY_ID.toString()))) {
						...
						if (dbobj.getString(FieldType.TXNTYPE.toString()).equalsIgnoreCase(TransactionType.RECO.getName()) ||
								dbobj.getString(FieldType.TXNTYPE.toString()).equalsIgnoreCase(TransactionType.SALE.getName())) {
							
							analyticsDataCountSearchObj.setPaymentMethod(dbobj.getString(FieldType.PAYMENT_TYPE.toString()));
						...
						else if (dbobj.getString(FieldType.TXNTYPE.toString()).equalsIgnoreCase(TransactionType.REFUNDRECO.getName()) ||
								dbobj.getString(FieldType.TXNTYPE.toString()).equalsIgnoreCase(TransactionType.REFUND.getName())) {{
									...
									analyticsDataCountSearchObj.setAcquirerRefundSurcharge(tdrPojo.getAcquirerSurchargeAmount());
